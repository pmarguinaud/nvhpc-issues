
MODULE FIELD_1RM_FACTORY_MODULE


USE FIELD_BASIC_MODULE
USE FIELD_1RM_MODULE

USE PARKIND1, ONLY : JPRM, JPRD, JPIM, JPLM

IMPLICIT NONE

PRIVATE

INTERFACE FIELD_NEW
  MODULE PROCEDURE FIELD_1RM_NEW_OWNER
  MODULE PROCEDURE FIELD_1RM_NEW_WRAPPER
END INTERFACE

INTERFACE


MODULE SUBROUTINE FIELD_1RM_RESIZE (FIELD_PTR, UBOUNDS, LBOUNDS, PERSISTENT)

CLASS(FIELD_1RM), POINTER :: FIELD_PTR
INTEGER(KIND=JPIM), INTENT(IN) :: UBOUNDS (1)
INTEGER(KIND=JPIM), OPTIONAL, INTENT(IN) :: LBOUNDS (1)
LOGICAL, OPTIONAL, INTENT(IN) :: PERSISTENT

END SUBROUTINE

END INTERFACE


PUBLIC :: FIELD_NEW

INTERFACE FIELD_DELETE
  MODULE PROCEDURE FIELD_1RM_DELETE
END INTERFACE FIELD_DELETE

PUBLIC :: FIELD_DELETE

INTERFACE FIELD_RESIZE
  MODULE PROCEDURE FIELD_1RM_RESIZE
END INTERFACE FIELD_RESIZE

PUBLIC :: FIELD_RESIZE

CONTAINS

SUBROUTINE FIELD_1RM_NEW_OWNER (FIELD_PTR, UBOUNDS, LBOUNDS, PERSISTENT, DELAYED, INIT_VALUE, PINNED, MAP_DEVPTR, &
&                                 POOLED)


CLASS(FIELD_1RM), POINTER :: FIELD_PTR
TYPE(FIELD_1RM_OWNER), POINTER :: FIELD_OWNER
INTEGER(KIND=JPIM), INTENT(IN) :: UBOUNDS (1)
INTEGER(KIND=JPIM), OPTIONAL, INTENT(IN) :: LBOUNDS (1)
LOGICAL, OPTIONAL, INTENT(IN) :: PERSISTENT
LOGICAL, OPTIONAL,  INTENT(IN) :: DELAYED
REAL(KIND=JPRM), OPTIONAL, INTENT(IN) :: INIT_VALUE
LOGICAL, OPTIONAL,  INTENT(IN) :: PINNED
LOGICAL, OPTIONAL,  INTENT(IN) :: MAP_DEVPTR
LOGICAL, OPTIONAL,  INTENT(IN) :: POOLED

END SUBROUTINE

SUBROUTINE FIELD_1RM_NEW_WRAPPER (FIELD_PTR, LBOUNDS, PERSISTENT, DATA, MAP_DEVPTR, SYNC_ON_FINAL, INITIALIZED)


CLASS(FIELD_1RM), POINTER :: FIELD_PTR
REAL(KIND=JPRM), TARGET, INTENT (IN) :: DATA (:)
TYPE(FIELD_1RM_WRAPPER), POINTER :: FIELD_WRAPPER
INTEGER(KIND=JPIM), OPTIONAL, INTENT(IN) :: LBOUNDS (1)
LOGICAL, OPTIONAL, INTENT(IN) :: PERSISTENT
LOGICAL, OPTIONAL,  INTENT(IN) :: MAP_DEVPTR
LOGICAL, OPTIONAL,  INTENT(IN) :: SYNC_ON_FINAL
LOGICAL, OPTIONAL,  INTENT(IN) :: INITIALIZED

END SUBROUTINE

SUBROUTINE FIELD_1RM_DELETE (FIELD_PTR)


CLASS(FIELD_1RM), POINTER :: FIELD_PTR

END SUBROUTINE


END MODULE

SUBMODULE(FIELD_1RM_FACTORY_MODULE) FIELD_1RM_FACTORY_RESIZE_MODULE

USE PARKIND1, ONLY : JPRM, JPRD, JPIM, JPLM

IMPLICIT NONE

CONTAINS

MODULE SUBROUTINE FIELD_1RM_RESIZE (FIELD_PTR, UBOUNDS, LBOUNDS, PERSISTENT)

CLASS(FIELD_1RM), POINTER :: FIELD_PTR
INTEGER(KIND=JPIM), INTENT(IN) :: UBOUNDS (1)
INTEGER(KIND=JPIM), OPTIONAL, INTENT(IN) :: LBOUNDS (1)
LOGICAL, OPTIONAL, INTENT(IN) :: PERSISTENT

END SUBROUTINE FIELD_1RM_RESIZE


END SUBMODULE
