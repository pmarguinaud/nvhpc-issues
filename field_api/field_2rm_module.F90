...
CONTAINS
  ...
  PROCEDURE :: SYNC_DEVICE_WRONLY => FIELD_2RM_SYNC_DEVICE_WRONLY
  PROCEDURE :: COPY_OBJECT => FIELD_2RM_COPY_OBJECT
  PROCEDURE :: WIPE_OBJECT => FIELD_2RM_WIPE_OBJECT
  PROCEDURE :: GET_DIMS => FIELD_2RM_GET_DIMS
...
!   thread has moved them meanwhile. And if they are still not, then they are moved.
  ...
  SUBROUTINE FIELD_2RM_SYNC_DEVICE_WRONLY (SELF, QUEUE)
    CLASS(FIELD_2RM) :: SELF
    INTEGER(KIND=JPIM), OPTIONAL, INTENT(IN)    :: QUEUE
    REAL(KIND=JPRM), POINTER :: PPTR(:,:)

  END SUBROUTINE FIELD_2RM_SYNC_DEVICE_WRONLY
