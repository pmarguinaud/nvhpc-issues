...
CONTAINS
  ...
  FUNCTION VARIABLE_2RB_INIT (NAME, CNAME, IGRBCODE, LACTIVE, LADV, LT1, LT9, LPH9, LDL, LDM, LDL9, LDM9, LPC_PH, LGP, LWATER,&
    ...
    REAL(KIND=JPRB) :: ZDEFAULT, ZARG ! for portability : it is not clear whether the first argument of IEEE_VALUE can be optional ...
    ...
    IF (PRESENT(RCP)) THEN
      ZDEFAULT=IEEE_VALUE (RCP, IEEE_QUIET_NAN)
    ELSE
      ZDEFAULT=IEEE_VALUE (ZARG, IEEE_QUIET_NAN)
    ENDIF
    SELF%RCP = ARGUMENT_VALUE(ARG=RCP, DEFAULT=ZDEFAULT)
    IF (PRESENT(R)) THEN
      ZDEFAULT=IEEE_VALUE (R, IEEE_QUIET_NAN)
    ELSE
      ZDEFAULT=IEEE_VALUE (ZARG, IEEE_QUIET_NAN)
    ENDIF
    SELF%R = ARGUMENT_VALUE(ARG=R, DEFAULT=ZDEFAULT)
