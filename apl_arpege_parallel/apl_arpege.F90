...
!     2025-01-05 J.M. Piriou provisional add of ICBOT_LIG for convective gusts. 
!     2025-06-19 J.M. Piriou : compute ICBOT_LIG from deep convection to protect convective gusts computation in mid-level convection situations.
! End Modifications
!-------------------------------------------------------------------------------
...
& PR=>YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, PT=> YDMF_PHYS_BASE_STATE%T%P, PU=> YDMF_PHYS_BASE_STATE%U%P,                                   &
& PV=>YDMF_PHYS_BASE_STATE%V%P, PCP=>YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP)
...
CALL CPPHINP(YDGEOMETRY, YDMODEL, KLON, KFLEVG, KIDIA, KFDIA, YDGEOMVARS%GEMU,  YDGEOMVARS%GELAM,                         &
...
IF (YDCPG_OPTS%LCONFX) THEN
  ...
  & ZSAV_UNEBH, ZSAV_MO, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VH%PPBLH, YDMF_PHYS_SURF%GSD_VH%PQSH, &
  ...
  & YDVARS%UNEBH%T0, YDVARS%UOM%T0, YDVARS%MO%T0, YDMODEL)
...
!$ACDC PARALLEL,NAME=CHECKMV {
  ...
  CALL CHECKMV (YDCPG_OPTS%NINDAT, YDMODEL%YRCST, YDRIP, YDPHY0, YDPHY2, KIDIA, KFDIA, KLON, KFLEVG, YDCPG_OPTS%NSTEP, &
    & PAPHI, PAPHIF, PAPRS, PAPRSF, YDGEOMVARS%GELAM, YDGEOMVARS%GEMU, ZRDG_MU0, YDMF_PHYS_SURF%GSD_VF%PLSM, PT, &
    & YDMF_PHYS_BASE_STATE%Q%P, YDMF_PHYS_BASE_STATE%YGSP_RR%T)
...
& YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDGEOMVARS, YDMODEL, IMOC_CLPH, INLAB_CVPP, ZAER, ZAERINDS, ZAIPCMT, ZALBD, &
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACSOL {
  ...
  IF ( LSOLV ) THEN
    ...
    & YDMF_PHYS_BASE_STATE%YGSP_RR%IC, LLHMT, ZDSA_C1, ZDSA_C2, ZC3, ZCG, ZCN, YDMF_PHYS%CT, ZNEIJG, ZNEIJV, ZWFC,     &
...
CALL APL_ARPEGE_INIT_SURFEX (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDGPAR, YDMF_PHYS, &
  & YDVARS, YDGEOMVARS, YDMODEL, ZALBD, ZALBP, ZFLU_EMIS, ZSGROUPEL, ZSRAIN, ZSSNOW, ZTSN)
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACHMTLS {
  ...
  & YDCPG_MISC%QS, ZDPHIT, YDMF_PHYS%GZ0, YDMF_PHYS%GZ0H, LLCLS, ZNBVNO, ZMRIPP, ZDSA_CPS, &
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACHMT {
  ...
  & ZMRIPP, ZFLU_CD, ZFLU_CDN, ZCDROV, ZFLU_CH, ZCHROV, ZDSA_CPS, ZDQSTS, ZGWDCS, YDMF_PHYS%GZ0,                   &
  & YDMF_PHYS%GZ0H, ZHQ, ZHU, ZFLU_NEIJ, YDMF_PHYS%QCLS, YDCPG_MISC%QS, ZFLU_QSATS, YDMF_PHYS%RHCLS,       &
  & ZDSA_RS, ZRTI, ZSTAB, YDMF_PHYS%TCLS, YDMF_PHYS%UCLS, YDMF_PHYS%VCLS, YDMF_PHYS%NUCLS,             &
  & YDMF_PHYS%NVCLS, ZPCLS, ZFLU_VEG, ZXDROV, ZXHROV, YDMF_PHYS%UGST, YDMF_PHYS%VGST)
...
& KFLEVG, ZTHETAV, PAPHI, PAPHIF, PU, PV, ZTHETAVS, IMOC_CLPH, YDMF_PHYS%CLPH)

DO JLON = KIDIA, KFDIA
  ZBLH(JLON) = YDMF_PHYS%CLPH(JLON)
ENDDO

CALL ACPBL_WIND(YDMODEL%YRCST, YDPHY2, KIDIA, KFDIA, KLON, YDCPG_OPTS%KTDIA, KFLEVG, &
 & PAPHI, PAPHIF, PU, PV, ZBLH, &
 & IMOC_CLPH, YDMF_PHYS%VEIN, ZUGST, ZVGST)
...
CALL APL_WIND_GUST (YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS, YDVARS, YDGEOMVARS, YDXFU, YDMODEL, &
...
& YDVARS, YDGEOMVARS, YDMODEL, LLREDPR, ZAIPCMT, ZBLH, ZDECRD, ZFLU_QSAT, ZMSC_QW, ZNEBC0, ZNEBCH, ZNEBS, ZNEBS0, &
& ZNEB_CVPP, ZPFL_FPLCH, ZQI, ZQL, ZQLIS, ZQLIS0, ZQLI_CVP, ZQLI_CVPP, ZQV, ZUNEBH, ZCLCT_RAD, YDSTA)
  
CALL APL_ARPEGE_RADIATION (YDMF_PHYS_BASE_STATE, YDCFU, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDGPAR, &
  & YDMF_PHYS, YDTRC, YDRADF, YDMF_PHYS_SURF, YDVARS, YDGEOMVARS, YDMODEL, ZCLCT_RAD, ZALBDIR, ZDECRD, ZAER, ZAERINDS, &
  & ZALBD, ZALBP, ZQCO2, ZCEMTR, ZCTRSO, ZFLU_EMIS, ZFLU_QSAT, ZQO3, ZQR, ZQS, ZQV, ZRDG_MU0, ZRDG_SOLO, ZRDG_MU0LU, &
...
CALL APL_ARPEGE_SURFACE (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDGPAR, YDMF_PHYS, &
  & YDMF_PHYS_SURF, YDVARS, YDGEOMVARS, YDMODEL, ZALBD, ZALBP, ZALPHA1, ZCDROV, ZCEROV, ZCFATH, ZCFAU, ZCFBTH, ZCFBU, &
  & ZCFBV, ZCHROV, ZCOEFA, ZCOEFN, ZCP, ZDIFEXT, ZDIFWQ, ZDIFWS, ZDQSTS, ZDSA_CPS, ZDSA_LHS, ZDTMSE, ZEDMFQ, ZEDMFS, &
  & ZEDMFU, ZEDMFV, ZFLU_CD, ZFLU_CDN, ZFLU_EMIS, ZFLU_FEVI, ZFLU_NEIJ, ZFLU_QSATS, ZFLU_VEG, ZHQ, ZHTR, ZHU, ZKQLROV, &
  & ZKQROV, ZKTROV, ZKUROV, ZLVT, ZMF_UP, ZNEBCH, ZNEBDIFF, ZNEBS, ZPOID, ZQI, ZQICE, ZQL, ZQV, ZRDG_MU0, ZRDG_MU0N, &
  & ZRHGMT, ZSC_FCLL, ZSC_FCLN, ZSC_FEVI, ZSC_FEVN, ZSFSWDIF, ZSFSWDIR, ZSGROUPEL, ZSRAIN, ZSSNOW, ZSTATI, ZTSN, ZXDROV, &
  & ZXHROV, ZXQRO, ZXTRO, ZXTROV, ZXURO, ZXUROV)
...
IF (LCVPPKF.OR.(LEDKF .AND. .NOT. LEDMFI)) THEN
   IF (LCVPPKF) THEN
    ...
    ELSEIF(LEDKF) THEN  
      DO JLEV=YDCPG_OPTS%KTDIA,KFLEVG
         DO JLON = KIDIA, KFDIA
            YDMF_PHYS%DIFTQ   (JLON,JLEV) = YDMF_PHYS%DIFTQ(JLON,JLEV) + ZEDMFQ(JLON,JLEV)
            YDMF_PHYS%DIFTS   (JLON,JLEV) = YDMF_PHYS%DIFTS(JLON,JLEV) + ZEDMFS(JLON,JLEV)
            YDMF_PHYS%STRTU   (JLON,JLEV) = YDMF_PHYS%STRTU(JLON,JLEV) + ZEDMFU(JLON,JLEV)
            YDMF_PHYS%STRTV   (JLON,JLEV) = YDMF_PHYS%STRTV(JLON,JLEV) + ZEDMFV(JLON,JLEV)
         ENDDO
      ENDDO
    ENDIF
ENDIF  
...
!DEC$ IVDEP
DO JLON = KIDIA, KFDIA
  IF(.NOT.LMSE)YDMF_PHYS%FRTHDS(JLON)=YDMF_PHYS%FRTH(JLON,KFLEVG,1)/ZFLU_EMIS(JLON)+YDMODEL%YRCST%RSIGMA*YDMF_PHYS_BASE_STATE%YGSP_RR%T(JLON)**4
  IF(ZRDG_MU0(JLON) <= 0.0_JPRB) THEN
    YDMF_PHYS%FRSOPT(JLON)=0.0_JPRB
  ELSE
    YDMF_PHYS%FRSOPT(JLON)=RII0*ZRDG_MU0(JLON)
  ENDIF
ENDDO
...
IF(LMCC03)THEN
  ...
  & ZCHROV, ZDQSTS, YDMF_PHYS%DRNSHF)
ENDIF
...
& PU, PV, YDGEOMVARS%RCORI, YDMF_PHYS_SURF%GSD_VF%PGETRL, ZGWDCS, YDMF_PHYS_SURF%GSD_VF%PVRLAN, &
& YDMF_PHYS_SURF%GSD_VF%PVRLDI, YDMF_PHYS%STRDU, YDMF_PHYS%STRDV, ZTRAJGWD)
  
IF ( LSTRAS ) THEN
  CALL ACPLUIS (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, KIDIA, KFDIA, KLON,                                          &
  & NTPLUI, KFLEVG, PAPRSF, PDELP, ZNEBS, ZQV, ZQLIS, ZMSC_QW, PR, PT, YDMF_PHYS%FCSQL,                      &
  & YDMF_PHYS%FCSQN, YDMF_PHYS%FPLSL, YDMF_PHYS%FPLSN, YDMF_PHYS%FPEVPSL, YDMF_PHYS%FPEVPSN, &
  & YDMF_PHYS%FPFPSL, YDMF_PHYS%FPFPSN                                                                   &
  &                                                                       )
ENDIF
...
IF (LCVTDK) THEN  
  CALL APL_ARPEGE_DEEP_CONVECTION (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS, YDCPG_DYN0, &
    & YDMF_PHYS_SURF, YDVARS, YDGEOMVARS, YDCFU, YDMODEL, ZFPCOR, ZQV, ZTENHA, ZTENQVA, ZTENT, ZRDG_CVGQ, ZRDG_CVGT,ICBOT_LIG)
ELSE

!$ACDC PARALLEL {

  ICBOT_LIG(:)=YDCPG_OPTS%KFLEVG
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=TURBULENCE_PART2 {
  ...
  & PAPHI, PAPHIF, PAPRS, PAPRSF, PDELP, PR, PCP, PT, YDMF_PHYS_BASE_STATE%Q%P, PU, PV, YDMF_PHYS_BASE_STATE%TKE%P, &
  & ZPRODTH, ZKUROV2, YDMF_PHYS%DIFCQ, YDMF_PHYS%DIFCS, YDMF_PHYS%STRCU, YDMF_PHYS%STRCV,       &
  & ZUSLE, ZKCLS, ZECTCLS, ZPFL_FTKE, ZTKE1, YDMF_PHYS%EDR)
...
& YDMF_PHYS_SURF, YDVARS, YDGEOMVARS, YDMODEL, ZFLU_NEIJ, ZNEBS, ZNEB_CVPP, ZQC_DET_PCMT, ZQI, ZQL, ZQLIS, &
...
IF(LRRGUST) THEN
  DO JLEV=YDCPG_OPTS%KTDIA-1,KFLEVG
    DO JLON = KIDIA, KFDIA
      ZPFL_FPLSH(JLON,JLEV)=YDMF_PHYS%FPLSL(JLON,JLEV)+YDMF_PHYS%FPLSN(JLON,JLEV)+YDMF_PHYS%FPLSG(JLON,JLEV)
    ENDDO
  ENDDO
ENDIF
...
& ZQV, ZQL, ZQI, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS%DIFCQ, YDMF_PHYS%DIFCQN,                &
& YDMF_PHYS%DIFCQL, YDMF_PHYS%DIFTQ, YDMF_PHYS%DIFTQN, YDMF_PHYS%DIFTQL, YDMF_PHYS%FPEVPSL,     &
& YDMF_PHYS%FPEVPSN, YDMF_PHYS%FPEVPCL, YDMF_PHYS%FPEVPCN, YDMF_PHYS%FPFPSL, YDMF_PHYS%FPFPSN,  &
& YDMF_PHYS%FPFPCL, YDMF_PHYS%FPFPCN, YDMF_PHYS%FCCQL, YDMF_PHYS%FCCQN, YDMF_PHYS%FCSQL,        &
& YDMF_PHYS%FCSQN, YDMF_PHYS%FCQNG )

!$ACDC }

! Bilan hydrique du sol

CALL APL_ARPEGE_HYDRO_BUDGET (YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDGPAR, YDMF_PHYS, &
...
& NTDRME, KFLEVG, PCP, PDELP, PT, ZQV, PU, PV, YDMF_PHYS%FRMH, ZMSC_FRMQ, YDMF_PHYS%STRMU, &
& YDMF_PHYS%STRMV)
...
! DCAPE due to precipitation evaporation.
  ...
  & KFLEVG, YDMF_PHYS%FPLSL, YDMF_PHYS%FPLSN, YDMF_PHYS%FPLCL, YDMF_PHYS%FPLCN, &
  & PAPRS, PAPRSF, PT, PCP, PAPHIF, PAPHI, ZDCAPE)

! Gusts.

  CALL ACCLDIA (YDMODEL%YRCST, YDXFU%LXCLP, YDXFU%LXTGST, YDXFU%LXXGST, YDPHY, YDPHY2, YDTOPH, KIDIA, KFDIA, KLON, &
    & KFLEVG, YDMF_PHYS%UCLS, YDMF_PHYS%VCLS, PU, PV, YDMF_PHYS%CAPE, ICBOT_LIG, ZDCAPE, ZTKE1, PAPHIF, YDGEOMVARS%OROG, &
    & YDMF_PHYS%UGST, YDMF_PHYS%VGST, ZBLH, IMOC_CLPH)

  DO JLON = KIDIA, KFDIA
    YDMF_PHYS%CLPH(JLON)=MIN(XMAXLM,MAX(XMINLM,ZBLH(JLON)))
  ENDDO
...
& ZQR, ZQS, ZQGM, YDMF_PHYS%VISICLD, YDMF_PHYS%VISIHYD, YDMF_PHYS%MXCLWC)
...
DO JLEV=1,KFLEVG
  DO JLON = KIDIA, KFDIA
    ZBAY_QRCONV(JLON,JLEV)  = 0.001_JPRB * ( (ABS(YDMF_PHYS%FPLCL(JLON,JLEV)) * YDPHY0%RCOEFRAIN(1)) ** YDPHY0%RCOEFRAIN(2) )
    ZBAY_QSCONV(JLON,JLEV)  = 0.001_JPRB * ( (ABS(YDMF_PHYS%FPLCN(JLON,JLEV)) * YDPHY0%RCOEFSNOW(1)) ** YDPHY0%RCOEFSNOW(2) )    
  ENDDO
ENDDO
...
& YDMF_PHYS%TCLS,  YDMF_PHYS_BASE_STATE%Q%P(:, KFLEVG), YDMF_PHYS_BASE_STATE%L%P(:, KFLEVG), &
& YDMF_PHYS_BASE_STATE%I%P(:, KFLEVG),  YDMF_PHYS%TPWCLS)

!$ACDC }

CALL APL_ARPEGE_DPRECIPS (YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS, YDVARS, YDGEOMVARS, YDMODEL, &
...
!$ACDC PARALLEL {
  ...
  CALL MF_PHYS_CORWAT (YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS%FEVL, YDMF_PHYS%FEVN, YDMF_PHYS%FPLCL,                &
  & YDMF_PHYS%FPLCN, YDMF_PHYS%FPLSL, YDMF_PHYS%FPLSN, YDMF_PHYS_SURF%GSD_VH%PEVA, YDMF_PHYS_SURF%GSD_VH%PPCL, &
...
& YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDMF_PHYS, YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS, YDGEOMVARS, YDMODEL, YDDDH, &
...
CALL APL_ARPEGE_SURFACE_UPDATE (YDCPG_BNDS, YDCPG_OPTS, YDGPAR, YDMF_PHYS, YDMF_PHYS_SURF, &
...
IF (YDCPG_OPTS%LCONFX) THEN
  ...
  CALL TRANSFER_CLOUD_FIELDS(YDMODEL, YDVARS, YDCPG_OPTS, YDCPG_BNDS)
...
IF (LEDR) THEN
  DO JLEV = 1, YDCPG_OPTS%YRSURF_DIMS%YSD_DID%NLEVS
    YDMF_PHYS_SURF%GSD_DI%PXEDR(:,JLEV)=YDMF_PHYS%EDR(:,JLEV)
  ENDDO
ENDIF
