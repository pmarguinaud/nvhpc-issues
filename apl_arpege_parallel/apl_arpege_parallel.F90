SUBROUTINE APL_ARPEGE_PARALLEL (YDMF_PHYS_BASE_STATE, YDMF_PHYS_NEXT_STATE, YDGEOMETRY, YDCPG_BNDS&
&, YDCPG_OPTS, YDCPG_MISC, YDGPAR, YDCPG_PHY0, YDMF_PHYS, YDTRC, YDRADF, YDCPG_DYN0, YDMF_PHYS_SURF&
&, YDCPG_SL2, YDVARS, YDGEOMVARS, YDCFU, YDXFU, YDMODEL, YDDDH, YDPHYSMWAVE)
...
#include "acdrag.h"
#include "acdrme.h"
#include "acevadcape.h"
#include "achmt.h"
#include "achmtls.h"
#include "acpbl_wind.h"
#include "acpluis.h"
#include "acsol.h"
#include "actqsat.h"
#include "acvisih.h"
#include "aplpar_init.h"
#include "checkmv.h"
#include "cpphinp.h"
#include "cpqsol.h"
#include "mf_phys_bayrad.h"
#include "mf_phys_corwat.h"
#include "mf_phys_cvv.h"
#include "mf_phys_fpl_part1.h"
#include "mf_phys_fpl_part2.h"
#include "mf_phys_mocon.h"
#include "mf_phys_precips.h"
#include "mf_phys_save_phsurf_part1.h"
#include "mf_phys_save_phsurf_part2.h"
#include "mf_phys_transfer.h"
#include "transfer_cloud_fields.h"
#include "ppwetpoint.h"
#include "qngcor.h"
#include "aplpar_flexdia.h"
#include "apl_arpege_oceanic_fluxes.h"
#include "apl_wind_gust.h"
#include "apl_arpege_shallow_convection_and_turbulence.h"
#include "apl_arpege_albedo_computation.h"
#include "apl_arpege_aerosols_for_radiation.h"
#include "apl_arpege_cloudiness.h"
#include "apl_arpege_radiation.h"
#include "apl_arpege_soil_hydro.h"
#include "apl_arpege_surface.h"
#include "apl_arpege_deep_convection.h"
#include "apl_arpege_turbulence_part2.h"
#include "apl_arpege_precipitation.h"
#include "apl_arpege_hydro_budget.h"
#include "apl_arpege_surface_update.h"
#include "apl_arpege_atmosphere_update.h"
#include "apl_arpege_init.h"
#include "apl_arpege_init_surfex.h"
#include "apl_arpege_dprecips.h"
#include "apl_arpege_mwave.h"
...
CLASS (FIELD_3RB), POINTER :: YL_ZSAV_MO
REAL (KIND=JPRB), POINTER::ZSAV_MO (:,:,:)
...
REAL (KIND=JPRB), POINTER :: Z_YDGEOMVARS_GELAM(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDGEOMVARS_GEMU(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDGEOMVARS_OROG(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDGEOMVARS_RCORI(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_I_P(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_L_P(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_Q_P(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_TKE_P(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_T_P(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_U_P(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_V_P(:,:,:)
...
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_CAPE(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_CLPH(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_CT(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_DIFCQ(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_DIFCQL(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_DIFCQN(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_DIFCS(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_DIFTQ(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_DIFTQL(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_DIFTQN(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_DIFTS(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_DRNSHF(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_EDR(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FCCQL(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FCCQN(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FCQNG(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FCSQL(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FCSQN(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FEVL(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FEVN(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPEVPCL(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPEVPCN(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPEVPSL(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPEVPSN(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPFPCL(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPFPCN(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPFPSL(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPFPSN(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPLCL(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPLCN(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPLSG(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPLSL(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FPLSN(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FRMH(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FRSOPT(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FRTH(:,:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_FRTHDS(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_GZ0(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_GZ0H(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_MXCLWC(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_NUCLS(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_NVCLS(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_QCLS(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_RHCLS(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_STRCU(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_STRCV(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_STRDU(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_STRDV(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_STRMU(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_STRMV(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_STRTU(:,:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_STRTV(:,:,:)
...
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_TCLS(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_TPWCLS(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_UCLS(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_UGST(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_VCLS(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_VEIN(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_VGST(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_VISICLD(:,:)
REAL (KIND=JPRB), POINTER :: Z_YDMF_PHYS_VISIHYD(:,:)
...
CALL FXTRAN_ACDC_GPUMEMSTAT ("apl_arpege_parallel.F90", 1, "BEGIN")
...
&%JBLKMAX], LBOUNDS=[1, YDCPG_OPTS%JBLKMIN], PERSISTENT=.TRUE.)
CALL FIELD_NEW (YL_ZSAV_MO, UBOUNDS=[YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS&
&%JBLKMAX], LBOUNDS=[1, 1, YDCPG_OPTS%JBLKMIN], PERSISTENT=.TRUE.)
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:CPPHINP')) THEN
  ...
  Z_YDGEOMVARS_GELAM => GET_HOST_DATA_RDONLY (YDGEOMVARS%F_GELAM)
  Z_YDGEOMVARS_GEMU => GET_HOST_DATA_RDONLY (YDGEOMVARS%F_GEMU)
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    &%KFDIA, Z_YDGEOMVARS_GEMU(:, JBLK), Z_YDGEOMVARS_GELAM(:, JBLK), Z_YDVARS_U_T0(:,:, JBLK), Z_YDVARS_V_T0&
    &(:,:, JBLK), Z_YDVARS_T_DL(:,:, JBLK), Z_YDVARS_T_DM(:,:, JBLK), Z_YDVARS_Q_T0(:,:, JBLK), Z_YDVARS_Q_DL&
    &(:,:, JBLK), Z_YDVARS_Q_DM(:,:, JBLK), Z_YDVARS_CVGQ_DL(:,:, JBLK), Z_YDVARS_CVGQ_DM(:,:, JBLK&
    &), Z_YDCPG_PHY0_XYB_RDELP(:,:, JBLK), Z_YDCPG_DYN0_CTY_EVEL(:,:, JBLK), Z_YDVARS_CVGQ_T0(:,:,&
  ...
  IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:CPPHINP')) THEN
    ...
    Z_YDGEOMVARS_GELAM => GET_HOST_DATA_RDWR (YDGEOMVARS%F_GELAM)
    Z_YDGEOMVARS_GEMU => GET_HOST_DATA_RDWR (YDGEOMVARS%F_GEMU)
  ...
  Z_YDGEOMVARS_GELAM => NULL ()
  Z_YDGEOMVARS_GEMU => NULL ()
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:CPPHINP')) THEN
  ...
  Z_YDGEOMVARS_GELAM => GET_DEVICE_DATA_RDONLY (YDGEOMVARS%F_GELAM)
  Z_YDGEOMVARS_GEMU => GET_DEVICE_DATA_RDONLY (YDGEOMVARS%F_GEMU)
  ...
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:CPPHINP:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         Z_YDCPG_DYN0_CTY_EVEL, Z_YDCPG_PHY0_XYB_RDELP, Z_YDGEOMVARS_GELAM, &
    !$ACC&         Z_YDGEOMVARS_GEMU, Z_YDVARS_CVGQ_DL, Z_YDVARS_CVGQ_DM, Z_YDVARS_CVGQ_T0, &
...
IF (YDCPG_OPTS%LCONFX) THEN
  ...
  &, YL_ZSAV_FHPS, YL_ZSAV_GZ0F, YL_ZSAV_GZ0HF, YL_ZSAV_HV, YL_ZSAV_PBLH, YL_ZSAV_QSH, YL_ZSAV_UDAL, YL_ZSAV_UDGRO&
  &, YL_ZSAV_UDOM, YL_ZSAV_UNEBH, YL_ZSAV_MO, YDMF_PHYS_SURF%GSD_VF%F_Z0F, YDMF_PHYS_SURF%GSD_VH%F_PBLH&
  &, YDMF_PHYS_SURF%GSD_VH%F_QSH, YDMF_PHYS_SURF%GSD_VH%F_SPSH, YDMF_PHYS_SURF%GSD_VK%F_UDGRO, YDMF_PHYS_SURF&
  &%GSD_VV%F_HV, YDMF_PHYS_SURF%GSD_VV%F_Z0H, YDVARS%DAL%FT0, YDVARS%DOM%FT0, YDVARS%UAL%FT0, YDVARS%UEN&
  &%FT0, YDVARS%UNEBH%FT0, YDVARS%UOM%FT0, YDVARS%MO%FT0, YDMODEL)
ENDIF
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LGCHECKMV) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OpenMP','APL_ARPEGE_PARALLEL:CHECKMV')) THEN
    ...
    Z_YDGEOMVARS_GELAM => GET_HOST_DATA_RDONLY (YDGEOMVARS%F_GELAM)
    Z_YDGEOMVARS_GEMU => GET_HOST_DATA_RDONLY (YDGEOMVARS%F_GEMU)
    Z_YDMF_PHYS_BASE_STATE_Q_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%Q%F_P)
    Z_YDMF_PHYS_BASE_STATE_T_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%T%F_P)
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      &(:,:, JBLK), Z_YDGEOMVARS_GELAM(:, JBLK), Z_YDGEOMVARS_GEMU(:, JBLK), ZRDG_MU0(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PLSM&
      &(:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_Q_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T&
      &(:, JBLK))
    ENDDO
    ...
    IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:CHECKMV')) THEN
      ...
      Z_YDGEOMVARS_GELAM => GET_HOST_DATA_RDWR (YDGEOMVARS%F_GELAM)
      Z_YDGEOMVARS_GEMU => GET_HOST_DATA_RDWR (YDGEOMVARS%F_GEMU)
      Z_YDMF_PHYS_BASE_STATE_Q_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%Q%F_P)
      Z_YDMF_PHYS_BASE_STATE_T_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%T%F_P)
    ...
    Z_YDGEOMVARS_GELAM => NULL ()
    Z_YDGEOMVARS_GEMU => NULL ()
    Z_YDMF_PHYS_BASE_STATE_Q_P => NULL ()
    Z_YDMF_PHYS_BASE_STATE_T_P => NULL ()
...
&, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDGEOMVARS, YDMODEL, YL_IMOC_CLPH, YL_INLAB_CVPP, YL_ZAER, YL_ZAERINDS&
...
&, YL_ZDIFCVPPU, YL_ZDIFCVPPV, YL_ZDIFWQ, YL_ZDIFWS, YL_ZEDMFQ, YL_ZEDMFS, YL_ZEDMFU, YL_ZEDMFV, ZEPS0&
&, ZEPSNEB, YL_ZFPCOR, YL_ZKQLROV, YL_ZKQROV, YL_ZKTROV, YL_ZKUROV, YL_ZLVT, YL_ZMF_UP, YL_ZMRIPP&
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:ACTQSAT')) THEN
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:,:, JBLK), ZQV(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P&
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACTQSAT')) THEN
  ...
  Z_YDMF_PHYS_BASE_STATE_T_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%T%F_P)
  ...
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACTQSAT:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         ZMSC_LH, ZMSC_LSCPE, ZMSC_QW, ZMSC_TW, ZQV, Z_YDCPG_MISC_RH, Z_YDMF_PHYS_BASE_STATE_T_P, &
...
IF (.NOT.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:ACSOL')) THEN
    ...
    Z_YDMF_PHYS_CT => GET_HOST_DATA_RDWR (YDMF_PHYS%F_CT)
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VF%F_LSM)
    Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VF%F_VEG)
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LSOLV) THEN
        ...
        & Z_YDMF_PHYS_CT(:, JBLK), ZNEIJG(:, JBLK), ZNEIJV(:, JBLK), ZWFC(:, JBLK), ZWPMX(:, JBLK), ZWSEQ&
        &(:, JBLK), ZWSMX(:, JBLK), ZWWILT(:, JBLK))
      ELSE

        DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
          Z_YDMF_PHYS_CT (JLON, JBLK)=YDMODEL%YRML_PHY_MF%YRPHY1%HSOL/(1.0_JPRB+YDMODEL%YRML_PHY_MF%YRPHY1%HSOLIWR&
          &*(Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W (JLON, JBLK)+Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q (JLON, 1, JBLK))/&
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
        ...
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LSOLV) THEN
          ...
          &(:, JBLK), ZCG(:, JBLK), ZCN(:, JBLK), Z_YDMF_PHYS_CT(:, JBLK), ZNEIJG(:, JBLK), ZNEIJV(:, JBLK),&
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACSOL')) THEN
    ...
    Z_YDMF_PHYS_CT => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_CT)
    ...
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACSOL:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      ...
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R, Z_YDMF_PHYS_CT, Z_YDMF_PHYS_SURF_GSD_VF_PLSM, &
...
&, YDCPG_OPTS, YDCPG_MISC, YDGPAR, YDMF_PHYS, YDVARS, YDGEOMVARS, YDMODEL, YL_ZALBD&
&, YL_ZALBP, YL_ZFLU_EMIS, YL_ZSGROUPEL, YL_ZSRAIN, YL_ZSSNOW, YL_ZTSN)
...
IF (YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:ACHMTLS')) THEN
    ...
    Z_YDMF_PHYS_BASE_STATE_T_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%T%F_P)
    Z_YDMF_PHYS_BASE_STATE_U_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%U%F_P)
    Z_YDMF_PHYS_BASE_STATE_V_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%V%F_P)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
    ...
    & (YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    Z_YDMF_PHYS_GZ0 => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_GZ0)
    Z_YDMF_PHYS_GZ0H => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_GZ0H)
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF&
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(:,:, JBLK&
      &), Z_YDMF_PHYS_BASE_STATE_U_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T&
      &(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), ZDPHIT(:, JBLK), Z_YDMF_PHYS_GZ0(:, JBLK), Z_YDMF_PHYS_GZ0H(:&
      ...
      DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
        ...
        &, JBLK)*Z_YDMF_PHYS_BASE_STATE_T_P(JLON, YDCPG_OPTS%KFLEVG, JBLK)+YDMODEL%YRCST%RKAPPA*ZDPHI&
    ...
    IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:ACHMTLS')) THEN
      ...
      Z_YDMF_PHYS_BASE_STATE_T_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%T%F_P)
      Z_YDMF_PHYS_BASE_STATE_U_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%U%F_P)
      Z_YDMF_PHYS_BASE_STATE_V_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%V%F_P)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      ...
      & (YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_GZ0 => GET_HOST_DATA_RDWR (YDMF_PHYS%F_GZ0)
      Z_YDMF_PHYS_GZ0H => GET_HOST_DATA_RDWR (YDMF_PHYS%F_GZ0H)
    ...
    Z_YDMF_PHYS_BASE_STATE_T_P => NULL ()
    Z_YDMF_PHYS_BASE_STATE_U_P => NULL ()
    Z_YDMF_PHYS_BASE_STATE_V_P => NULL ()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL ()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL ()
    ...
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL ()
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL ()
    Z_YDMF_PHYS_GZ0 => NULL ()
    Z_YDMF_PHYS_GZ0H => NULL ()
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMPSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACHMTLS')) THEN
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
        ...
        &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF&
        &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(&
        &:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T&
        &(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), ZDPHIT(:, JBLK), Z_YDMF_PHYS_GZ0(:, JBLK), Z_YDMF_PHYS_GZ0H&
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACHMTLS')) THEN
    ...
    Z_YDMF_PHYS_BASE_STATE_T_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%T%F_P)
    Z_YDMF_PHYS_BASE_STATE_U_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%U%F_P)
    Z_YDMF_PHYS_BASE_STATE_V_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%V%F_P)
    ...
    & (YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    Z_YDMF_PHYS_GZ0 => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_GZ0)
    Z_YDMF_PHYS_GZ0H => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_GZ0H)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACHMTLS:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACHMTLS:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      ...
      !$ACC&         Z_YDCPG_MISC_QS, Z_YDMF_PHYS_BASE_STATE_T_P, Z_YDMF_PHYS_BASE_STATE_U_P, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_V_P, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, &
      ...
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_GZ0, Z_YDMF_PHYS_GZ0H) &
      !$ACC&PRIVATE (JBLK) &
      !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
ELSE
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:ACHMT')) THEN
    ...
    Z_YDMF_PHYS_GZ0 => GET_HOST_DATA_RDWR (YDMF_PHYS%F_GZ0)
    Z_YDMF_PHYS_GZ0H => GET_HOST_DATA_RDWR (YDMF_PHYS%F_GZ0H)
    Z_YDMF_PHYS_NUCLS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_NUCLS)
    Z_YDMF_PHYS_NVCLS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_NVCLS)
    Z_YDMF_PHYS_QCLS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_QCLS)
    Z_YDMF_PHYS_RHCLS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_RHCLS)
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VF%F_LSM)
    Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VF%F_VEG)
    Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_HOST_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VF%F_Z0F)
    Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_HOST_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
    ...
    Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_HOST_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VV%F_Z0H)
    Z_YDMF_PHYS_TCLS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_TCLS)
    Z_YDMF_PHYS_UCLS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_UCLS)
    Z_YDMF_PHYS_UGST => GET_HOST_DATA_RDWR (YDMF_PHYS%F_UGST)
    Z_YDMF_PHYS_VCLS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_VCLS)
    Z_YDMF_PHYS_VGST => GET_HOST_DATA_RDWR (YDMF_PHYS%F_VGST)
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      &%YRPHY1, YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRCST, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON&
      &, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%YRSURF_DIMS%YSD_VVD%NUMFLDS>=8.AND.YDMODEL%YRML_PHY_MF%YRPHY%LSOLV, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI&
      ...
      &(:,:, JBLK), ZQV(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P&
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK), ZPFL_FPLSH&
      &(:,:, JBLK), ZPFL_FPLCH(:,:, JBLK), ZDPHIT(:, JBLK), ZDPHIV(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0F(:&
      ...
      & ZFLU_CH(:, JBLK), ZCHROV(:, JBLK), ZDSA_CPS(:, JBLK), ZDQSTS(:, JBLK), ZGWDCS(:, JBLK), Z_YDMF_PHYS_GZ0&
      &(:, JBLK), Z_YDMF_PHYS_GZ0H(:, JBLK), ZHQ(:, JBLK), ZHU(:, JBLK), ZFLU_NEIJ(:, JBLK), Z_YDMF_PHYS_QCLS&
      &(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), ZFLU_QSATS(:, JBLK), Z_YDMF_PHYS_RHCLS(:, JBLK), ZDSA_RS(:, JBLK&
      &), ZRTI(:, JBLK), ZSTAB(:, JBLK), Z_YDMF_PHYS_TCLS(:, JBLK), Z_YDMF_PHYS_UCLS(:, JBLK), Z_YDMF_PHYS_VCLS&
      &(:, JBLK), Z_YDMF_PHYS_NUCLS(:, JBLK), Z_YDMF_PHYS_NVCLS(:, JBLK), ZPCLS(:, JBLK), ZFLU_VEG(:, JBLK)&
      &, ZXDROV(:, JBLK), ZXHROV(:, JBLK), Z_YDMF_PHYS_UGST(:, JBLK), Z_YDMF_PHYS_VGST(:, JBLK))
    ENDDO
    ...
    Z_YDMF_PHYS_GZ0 => NULL ()
    Z_YDMF_PHYS_GZ0H => NULL ()
    Z_YDMF_PHYS_NUCLS => NULL ()
    Z_YDMF_PHYS_NVCLS => NULL ()
    Z_YDMF_PHYS_QCLS => NULL ()
    Z_YDMF_PHYS_RHCLS => NULL ()
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL ()
    Z_YDMF_PHYS_SURF_GSD_VF_PVEG => NULL ()
    Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => NULL ()
    Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => NULL ()
    ...
    Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => NULL ()
    Z_YDMF_PHYS_TCLS => NULL ()
    Z_YDMF_PHYS_UCLS => NULL ()
    Z_YDMF_PHYS_UGST => NULL ()
    Z_YDMF_PHYS_VCLS => NULL ()
    Z_YDMF_PHYS_VGST => NULL ()
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMPSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACHMT')) THEN
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
        ...
        &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P&
        &(:,:, JBLK), ZPFL_FPLSH(:,:, JBLK), ZPFL_FPLCH(:,:, JBLK), ZDPHIT(:, JBLK), ZDPHIV(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0F&
        &(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PZ0H(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PHV&
        &(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK), ZNEIJG(:, JBLK), ZNEIJV(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F&
        & (:, 1, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK),&
        & ZWFC(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC(:, JBLK&
        ...
        &), Z_YDMF_PHYS_GZ0(:, JBLK), Z_YDMF_PHYS_GZ0H(:, JBLK), ZHQ(:, JBLK), ZHU(:, JBLK), ZFLU_NEIJ(:,&
        & JBLK), Z_YDMF_PHYS_QCLS(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), ZFLU_QSATS(:, JBLK), Z_YDMF_PHYS_RHCLS&
        &(:, JBLK), ZDSA_RS(:, JBLK), ZRTI(:, JBLK), ZSTAB(:, JBLK), Z_YDMF_PHYS_TCLS(:, JBLK), Z_YDMF_PHYS_UCLS&
        &(:, JBLK), Z_YDMF_PHYS_VCLS(:, JBLK), Z_YDMF_PHYS_NUCLS(:, JBLK), Z_YDMF_PHYS_NVCLS(:, JBLK), ZPCLS&
        &(:, JBLK), ZFLU_VEG(:, JBLK), ZXDROV(:, JBLK), ZXHROV(:, JBLK), Z_YDMF_PHYS_UGST(:, JBLK), Z_YDMF_PHYS_VGST&
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACHMT')) THEN
    ...
    Z_YDMF_PHYS_GZ0 => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_GZ0)
    Z_YDMF_PHYS_GZ0H => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_GZ0H)
    Z_YDMF_PHYS_NUCLS => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_NUCLS)
    Z_YDMF_PHYS_NVCLS => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_NVCLS)
    Z_YDMF_PHYS_QCLS => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_QCLS)
    Z_YDMF_PHYS_RHCLS => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_RHCLS)
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VF%F_LSM)
    Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VF%F_VEG)
    Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VF%F_Z0F)
    Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
    ...
    Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_SURF%GSD_VV%F_Z0H)
    Z_YDMF_PHYS_TCLS => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_TCLS)
    Z_YDMF_PHYS_UCLS => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_UCLS)
    Z_YDMF_PHYS_UGST => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_UGST)
    Z_YDMF_PHYS_VCLS => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_VCLS)
    Z_YDMF_PHYS_VGST => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_VGST)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACHMT:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACHMT:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      ...
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F, Z_YDMF_PHYS_GZ0, Z_YDMF_PHYS_GZ0H, &
      !$ACC&         Z_YDMF_PHYS_NUCLS, Z_YDMF_PHYS_NVCLS, Z_YDMF_PHYS_QCLS, Z_YDMF_PHYS_RHCLS, &
      !$ACC&         Z_YDMF_PHYS_SURF_GSD_VF_PLSM, Z_YDMF_PHYS_SURF_GSD_VF_PVEG, &
      ...
      !$ACC&         Z_YDMF_PHYS_TCLS, Z_YDMF_PHYS_UCLS, Z_YDMF_PHYS_UGST, Z_YDMF_PHYS_VCLS, &
      !$ACC&         Z_YDMF_PHYS_VGST) &
      !$ACC&PRIVATE (JBLK) &
      !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:ACCLPH')) THEN
  ...
  & (YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
  Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
  Z_YDMF_PHYS_CLPH => GET_HOST_DATA_RDWR (YDMF_PHYS%F_CLPH)
  Z_YDMF_PHYS_VEIN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_VEIN)
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JLEV=YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG
      ...
      DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
        ZTHETAV (JLON, JLEV, JBLK)=Z_YDMF_PHYS_BASE_STATE_T_P(JLON, JLEV, JBLK)*(1.0_JPRB&
    ...
    CALL ACCLPH (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY0, YDMODEL%YRML_PHY_MF%YRPHY2, YLCPG_BNDS&
    &%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, ZTHETAV&
    &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF&
    &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK&
    &), ZTHETAVS(:, JBLK), IMOC_CLPH(:, JBLK), Z_YDMF_PHYS_CLPH(:, JBLK))

    DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
      ZBLH (JLON, JBLK)=Z_YDMF_PHYS_CLPH (JLON, JBLK)
    ENDDO

    CALL ACPBL_WIND (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY2, YLCPG_BNDS%KIDIA, YLCPG_BNDS&
    &%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI&
    &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P&
    &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK), ZBLH(:, JBLK), IMOC_CLPH(:, JBLK&
    &), Z_YDMF_PHYS_VEIN(:, JBLK), ZUGST(:, JBLK), ZVGST(:, JBLK))
  ENDDO
  ...
  Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL ()
  Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL ()
  Z_YDMF_PHYS_CLPH => NULL ()
  Z_YDMF_PHYS_VEIN => NULL ()
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMPSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACCLPH')) THEN
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
      ...
      CALL ACCLPH_OPENACC (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY0, YDMODEL%YRML_PHY_MF%YRPHY2, YLCPG_BNDS&
      &%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, ZTHETAV(:,:,&
      & JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF&
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK), ZTHETAVS&
      &(:, JBLK), IMOC_CLPH(:, JBLK), Z_YDMF_PHYS_CLPH(:, JBLK),YDSTACK=YLSTACK)
      
      ZBLH (JLON, JBLK)=Z_YDMF_PHYS_CLPH (JLON, JBLK)
      
      CALL ACPBL_WIND_OPENACC (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY2, YLCPG_BNDS%KIDIA, YLCPG_BNDS&
      &%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI&
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P(&
      &:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK), ZBLH(:, JBLK), IMOC_CLPH(:, JBLK), Z_YDMF_PHYS_VEIN&
      &(:, JBLK), ZUGST(:, JBLK), ZVGST(:, JBLK),YDSTACK=YLSTACK)
    ENDDO
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACCLPH')) THEN
  ...
  & (YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
  Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
  Z_YDMF_PHYS_CLPH => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_CLPH)
  Z_YDMF_PHYS_VEIN => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_VEIN)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACCLPH:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACCLPH:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         ZQV, ZTHETAV, ZTHETAVS, ZUGST, ZVGST, Z_YDCPG_MISC_QS, Z_YDMF_PHYS_BASE_STATE_T_P, &
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_U_P, Z_YDMF_PHYS_BASE_STATE_V_P, &
    ...
    !$ACC&         Z_YDMF_PHYS_CLPH, Z_YDMF_PHYS_VEIN) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
CALL APL_WIND_GUST_PARALLEL (YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS&
&, YDVARS, YDGEOMVARS, YDXFU, YDMODEL, YL_IMOC_CLPH, YL_ZBLH, YL_ZDCAPE)
...
&, YDVARS, YDGEOMVARS, YDMODEL, LLREDPR, YL_ZAIPCMT, YL_ZBLH, YL_ZDECRD, YL_ZFLU_QSAT, YL_ZMSC_QW, YL_ZNEBC0&
...
CALL APL_ARPEGE_RADIATION_PARALLEL (YDMF_PHYS_BASE_STATE, YDCFU, YDGEOMETRY, YDCPG_BNDS&
&, YDCPG_OPTS, YDCPG_MISC, YDGPAR, YDMF_PHYS, YDTRC, YDRADF, YDMF_PHYS_SURF, YDVARS, YDGEOMVARS&
&, YDMODEL, YL_ZCLCT_RAD, YL_ZALBDIR, YL_ZDECRD, YL_ZAER, YL_ZAERINDS, YL_ZALBD, YL_ZALBP&
&, YL_ZQCO2, YL_ZCEMTR, YL_ZCTRSO, YL_ZFLU_EMIS, YL_ZFLU_QSAT, YL_ZQO3, YL_ZQR, YL_ZQS&
&, YL_ZQV, YL_ZRDG_MU0, YL_ZRDG_SOLO, YL_ZRDG_MU0LU, YL_ZRDG_MU0M, YL_ZSFSWDIF, YL_ZSFSWDIR&
&, YL_ZSUDU, YL_ZTENT, YL_ZTRSOD, YL_ZTRSODIF, YL_ZTRSODIR)
...
&, YDGPAR, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDGEOMVARS, YDMODEL, YL_ZALBD, YL_ZALBP, YL_ZALPHA1&
&, YL_ZCDROV, YL_ZCEROV, YL_ZCFATH, YL_ZCFAU, YL_ZCFBTH, YL_ZCFBU, YL_ZCFBV, YL_ZCHROV, YL_ZCOEFA&
&, YL_ZCOEFN, YL_ZCP, YL_ZDIFEXT, YL_ZDIFWQ, YL_ZDIFWS, YL_ZDQSTS, YL_ZDSA_CPS, YL_ZDSA_LHS, ZDTMSE&
&, YL_ZEDMFQ, YL_ZEDMFS, YL_ZEDMFU, YL_ZEDMFV, YL_ZFLU_CD, YL_ZFLU_CDN, YL_ZFLU_EMIS, YL_ZFLU_FEVI&
&, YL_ZFLU_NEIJ, YL_ZFLU_QSATS, YL_ZFLU_VEG, YL_ZHQ, YL_ZHTR, YL_ZHU, YL_ZKQLROV, YL_ZKQROV, YL_ZKTROV&
&, YL_ZKUROV, YL_ZLVT, YL_ZMF_UP, YL_ZNEBCH, YL_ZNEBDIFF, YL_ZNEBS, YL_ZPOID, YL_ZQI, YL_ZQICE, YL_ZQL&
&, YL_ZQV, YL_ZRDG_MU0, YL_ZRDG_MU0N, ZRHGMT, YL_ZSC_FCLL, YL_ZSC_FCLN, YL_ZSC_FEVI, YL_ZSC_FEVN&
&, YL_ZSFSWDIF, YL_ZSFSWDIR, YL_ZSGROUPEL, YL_ZSRAIN, YL_ZSSNOW, ZSTATI, YL_ZTSN, YL_ZXDROV, YL_ZXHROV&
&, YL_ZXQRO, YL_ZXTRO, YL_ZXTROV, YL_ZXURO, YL_ZXUROV)
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:DIFTQ')) THEN
  ...
  Z_YDMF_PHYS_DIFTQ => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFTQ)
  Z_YDMF_PHYS_DIFTS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFTS)
  Z_YDMF_PHYS_STRTU => GET_HOST_DATA_RDWR (YDMF_PHYS%F_STRTU)
  Z_YDMF_PHYS_STRTV => GET_HOST_DATA_RDWR (YDMF_PHYS%F_STRTV)
    ...
    IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVPPKF.OR.(YDMODEL%YRML_PHY_MF%YRPHY&
      ...
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVPPKF) THEN
        ...
        DO JLEV=YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG
          ...
          DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
            Z_YDMF_PHYS_DIFTQ (JLON, JLEV, JBLK)=Z_YDMF_PHYS_DIFTQ&
            & (JLON, JLEV, JBLK)+ZDIFCVPPQ (JLON, JLEV, JBLK)
            Z_YDMF_PHYS_DIFTS (JLON, JLEV, JBLK)=Z_YDMF_PHYS_DIFTS&
            & (JLON, JLEV, JBLK)+ZDIFCVPPS (JLON, JLEV, JBLK)
            Z_YDMF_PHYS_STRTU (JLON, JLEV, JBLK)=Z_YDMF_PHYS_STRTU&
            & (JLON, JLEV, JBLK)+ZDIFCVPPU (JLON, JLEV, JBLK)
            Z_YDMF_PHYS_STRTV (JLON, JLEV, JBLK)=Z_YDMF_PHYS_STRTV&
      ...
      ELSEIF (YDMODEL%YRML_PHY_MF%YRPHY%LEDKF) THEN
        ...
        DO JLEV=YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG
          ...
          DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
            Z_YDMF_PHYS_DIFTQ (JLON, JLEV, JBLK)=Z_YDMF_PHYS_DIFTQ (JLON, JLEV, JBLK)+ZEDMFQ (JLON, JLEV, JBLK)
            Z_YDMF_PHYS_DIFTS (JLON, JLEV, JBLK)=Z_YDMF_PHYS_DIFTS (JLON, JLEV, JBLK)+ZEDMFS (JLON, JLEV, JBLK)
            Z_YDMF_PHYS_STRTU (JLON, JLEV, JBLK)=Z_YDMF_PHYS_STRTU (JLON, JLEV, JBLK)+ZEDMFU (JLON, JLEV, JBLK)
            Z_YDMF_PHYS_STRTV (JLON, JLEV, JBLK)=Z_YDMF_PHYS_STRTV (JLON, JLEV, JBLK)+ZEDMFV (JLON, JLEV, JBLK)
          ENDDO
  ...
  ZEDMFU => NULL ()
  ZEDMFV => NULL ()
  Z_YDMF_PHYS_DIFTQ => NULL ()
  Z_YDMF_PHYS_DIFTS => NULL ()
  Z_YDMF_PHYS_STRTU => NULL ()
  Z_YDMF_PHYS_STRTV => NULL ()
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:DIFTQ')) THEN
  ...
  Z_YDMF_PHYS_DIFTQ => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_DIFTQ)
  Z_YDMF_PHYS_DIFTS => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_DIFTS)
  Z_YDMF_PHYS_STRTU => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_STRTU)
  Z_YDMF_PHYS_STRTV => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_STRTV)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:DIFTQ:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:DIFTQ:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         ZDIFCVPPU, ZDIFCVPPV, ZEDMFQ, ZEDMFS, ZEDMFU, ZEDMFV, Z_YDMF_PHYS_DIFTQ, &
    !$ACC&         Z_YDMF_PHYS_DIFTS, Z_YDMF_PHYS_STRTU, Z_YDMF_PHYS_STRTV) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:FRSOPT')) THEN
  ...
  ZRDG_MU0 => GET_HOST_DATA_RDONLY (YL_ZRDG_MU0)
  Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
  Z_YDMF_PHYS_FRSOPT => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FRSOPT)
  Z_YDMF_PHYS_FRTH => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FRTH)
  Z_YDMF_PHYS_FRTHDS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FRTHDS)
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
      ...
      IF (.NOT.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE)  THEN
        Z_YDMF_PHYS_FRTHDS (JLON, JBLK)=Z_YDMF_PHYS_FRTH (JLON, YDCPG_OPTS%KFLEVG, 1, JBLK)/ZFLU_EMIS&
        & (JLON, JBLK)+YDMODEL%YRCST%RSIGMA*Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T (JLON, JBLK)**4
      ENDIF


      IF (ZRDG_MU0 (JLON, JBLK)<=0.0_JPRB) THEN
        Z_YDMF_PHYS_FRSOPT (JLON, JBLK)=0.0_JPRB
      ELSE
        Z_YDMF_PHYS_FRSOPT (JLON, JBLK)=YDMODEL%YRML_PHY_MF%YRPHY3%RII0*ZRDG_MU0 (JLON, JBLK)
      ENDIF
  ...
  IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:FRSOPT')) THEN
    ...
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    Z_YDMF_PHYS_FRSOPT => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FRSOPT)
    Z_YDMF_PHYS_FRTH => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FRTH)
    Z_YDMF_PHYS_FRTHDS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FRTHDS)
  ...
  ZRDG_MU0 => NULL ()
  Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL ()
  Z_YDMF_PHYS_FRSOPT => NULL ()
  Z_YDMF_PHYS_FRTH => NULL ()
  Z_YDMF_PHYS_FRTHDS => NULL ()
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:FRSOPT')) THEN
  ...
  Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
  Z_YDMF_PHYS_FRSOPT => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FRSOPT)
  Z_YDMF_PHYS_FRTH => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FRTH)
  Z_YDMF_PHYS_FRTHDS => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FRTHDS)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:FRSOPT:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:FRSOPT:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_FRSOPT, Z_YDMF_PHYS_FRTH, &
    !$ACC&         Z_YDMF_PHYS_FRTHDS) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:ACDRAG')) THEN
  ...
  Z_YDCPG_MISC_QS => GET_HOST_DATA_RDONLY (YDCPG_MISC%F_QS)
  Z_YDGEOMVARS_RCORI => GET_HOST_DATA_RDONLY (YDGEOMVARS%F_RCORI)
  Z_YDMF_PHYS_BASE_STATE_T_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%T%F_P)
  Z_YDMF_PHYS_BASE_STATE_U_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%U%F_P)
  Z_YDMF_PHYS_BASE_STATE_V_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%V%F_P)
  Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDONLY&
  ...
  Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
  Z_YDMF_PHYS_DRNSHF => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DRNSHF)
  Z_YDMF_PHYS_FCSQL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FCSQL)
  Z_YDMF_PHYS_FCSQN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FCSQN)
  Z_YDMF_PHYS_FPEVPSL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPEVPSL)
  Z_YDMF_PHYS_FPEVPSN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPEVPSN)
  Z_YDMF_PHYS_FPFPSL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPFPSL)
  Z_YDMF_PHYS_FPFPSN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPFPSN)
  Z_YDMF_PHYS_FPLSL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLSL)
  Z_YDMF_PHYS_FPLSN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLSN)
  Z_YDMF_PHYS_STRDU => GET_HOST_DATA_RDWR (YDMF_PHYS%F_STRDU)
  Z_YDMF_PHYS_STRDV => GET_HOST_DATA_RDWR (YDMF_PHYS%F_STRDV)
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    IF (YDMODEL%YRML_AOC%YRMCC%LMCC03) THEN
      CALL ACDNSHF (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY1, YLCPG_BNDS%KIDIA, YLCPG_BNDS&
      &%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, ZFLU_EMIS(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PLSM&
      &(:, JBLK), ZFLU_NEIJ(:, JBLK), ZQV(:,:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T&
      &(:, JBLK), ZCHROV(:, JBLK), ZDQSTS(:, JBLK), Z_YDMF_PHYS_DRNSHF(:, JBLK))
    ENDIF
    ...
    &(:,:, JBLK), ZNBVNO(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P&
    &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK), Z_YDGEOMVARS_RCORI(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PGETRL&
    &(:, JBLK), ZGWDCS(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI&
    &(:, JBLK), Z_YDMF_PHYS_STRDU(:,:, JBLK), Z_YDMF_PHYS_STRDV(:,:, JBLK), ZTRAJGWD(:,:, JBLK)&
    &)
    ...
    IF (YDMODEL%YRML_PHY_MF%YRPHY%LSTRAS) THEN
      ...
      &:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:,:, JBLK), ZNEBS(:,:, JBLK), ZQV(:,:, JBLK&
      &), ZQLIS(:,:, JBLK), ZMSC_QW(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P&
      &(:,:, JBLK), Z_YDMF_PHYS_FCSQL(:,:, JBLK), Z_YDMF_PHYS_FCSQN(:,:, JBLK), Z_YDMF_PHYS_FPLSL(:,:, JBLK&
      &), Z_YDMF_PHYS_FPLSN(:,:, JBLK), Z_YDMF_PHYS_FPEVPSL(:,:, JBLK), Z_YDMF_PHYS_FPEVPSN(:,:, JBLK),&
      & Z_YDMF_PHYS_FPFPSL(:,:, JBLK), Z_YDMF_PHYS_FPFPSN(:,:, JBLK))
    ENDIF
  ...
  IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:ACDRAG')) THEN
    ...
    ZTRAJGWD => GET_HOST_DATA_RDWR (YL_ZTRAJGWD)
    Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR (YDCPG_MISC%F_QS)
    Z_YDGEOMVARS_RCORI => GET_HOST_DATA_RDWR (YDGEOMVARS%F_RCORI)
    Z_YDMF_PHYS_BASE_STATE_T_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%T%F_P)
    Z_YDMF_PHYS_BASE_STATE_U_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%U%F_P)
    Z_YDMF_PHYS_BASE_STATE_V_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%V%F_P)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
  ...
  Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL ()
  Z_YDMF_PHYS_DRNSHF => NULL ()
  Z_YDMF_PHYS_FCSQL => NULL ()
  Z_YDMF_PHYS_FCSQN => NULL ()
  Z_YDMF_PHYS_FPEVPSL => NULL ()
  Z_YDMF_PHYS_FPEVPSN => NULL ()
  Z_YDMF_PHYS_FPFPSL => NULL ()
  Z_YDMF_PHYS_FPFPSN => NULL ()
  Z_YDMF_PHYS_FPLSL => NULL ()
  Z_YDMF_PHYS_FPLSN => NULL ()
  Z_YDMF_PHYS_STRDU => NULL ()
  Z_YDMF_PHYS_STRDV => NULL ()
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
      ...
      IF (YDMODEL%YRML_AOC%YRMCC%LMCC03) THEN
        ...
        &(:, JBLK), ZDQSTS(:, JBLK), Z_YDMF_PHYS_DRNSHF(:, JBLK),YDSTACK=YLSTACK)
      ENDIF
      ...
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK), Z_YDGEOMVARS_RCORI(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PGETRL&
      &(:, JBLK), ZGWDCS(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI&
      &(:, JBLK), Z_YDMF_PHYS_STRDU(:,:, JBLK), Z_YDMF_PHYS_STRDV(:,:, JBLK), ZTRAJGWD(:,:, JBLK),YDSTACK&
      &=YLSTACK)

      IF (YDMODEL%YRML_PHY_MF%YRPHY%LSTRAS) THEN
        CALL ACPLUIS_OPENACC (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS&
        &%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF&
        &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:,:, JBLK), ZNEBS(:,:, JBLK), ZQV(:,:, JBLK),&
        & ZQLIS(:,:, JBLK), ZMSC_QW(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P&
        &(:,:, JBLK), Z_YDMF_PHYS_FCSQL(:,:, JBLK), Z_YDMF_PHYS_FCSQN(:,:, JBLK), Z_YDMF_PHYS_FPLSL(:,:, JBLK&
        &), Z_YDMF_PHYS_FPLSN(:,:, JBLK), Z_YDMF_PHYS_FPEVPSL(:,:, JBLK), Z_YDMF_PHYS_FPEVPSN(:,:, JBLK), Z_YDMF_PHYS_FPFPSL&
        &(:,:, JBLK), Z_YDMF_PHYS_FPFPSN(:,:, JBLK),YDSTACK=YLSTACK)
      ENDIF
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACDRAG')) THEN
  ...
  Z_YDCPG_MISC_QS => GET_DEVICE_DATA_RDONLY (YDCPG_MISC%F_QS)
  Z_YDGEOMVARS_RCORI => GET_DEVICE_DATA_RDONLY (YDGEOMVARS%F_RCORI)
  Z_YDMF_PHYS_BASE_STATE_T_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%T%F_P)
  Z_YDMF_PHYS_BASE_STATE_U_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%U%F_P)
  Z_YDMF_PHYS_BASE_STATE_V_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%V%F_P)
  Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_DEVICE_DATA_RDONLY&
  ...
  & (YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
  Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
  Z_YDMF_PHYS_DRNSHF => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_DRNSHF)
  Z_YDMF_PHYS_FCSQL => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FCSQL)
  Z_YDMF_PHYS_FCSQN => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FCSQN)
  Z_YDMF_PHYS_FPEVPSL => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FPEVPSL)
  Z_YDMF_PHYS_FPEVPSN => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FPEVPSN)
  Z_YDMF_PHYS_FPFPSL => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FPFPSL)
  Z_YDMF_PHYS_FPFPSN => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FPFPSN)
  Z_YDMF_PHYS_FPLSL => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FPLSL)
  Z_YDMF_PHYS_FPLSN => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FPLSN)
  Z_YDMF_PHYS_STRDU => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_STRDU)
  Z_YDMF_PHYS_STRDV => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_STRDV)
  ...
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACDRAG:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         Z_YDGEOMVARS_RCORI, Z_YDMF_PHYS_BASE_STATE_T_P, Z_YDMF_PHYS_BASE_STATE_U_P, &
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_V_P, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R, &
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, &
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP, &
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_DRNSHF, Z_YDMF_PHYS_FCSQL, &
    !$ACC&         Z_YDMF_PHYS_FCSQN, Z_YDMF_PHYS_FPEVPSL, Z_YDMF_PHYS_FPEVPSN, &
    !$ACC&         Z_YDMF_PHYS_FPFPSL, Z_YDMF_PHYS_FPFPSN, Z_YDMF_PHYS_FPLSL, &
    !$ACC&         Z_YDMF_PHYS_FPLSN, Z_YDMF_PHYS_STRDU, Z_YDMF_PHYS_STRDV, &
    !$ACC&         Z_YDMF_PHYS_SURF_GSD_VF_PGETRL, Z_YDMF_PHYS_SURF_GSD_VF_PLSM, &
    !$ACC&         Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN, Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVTDK) THEN
  CALL APL_ARPEGE_DEEP_CONVECTION_PARALLEL (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS&
  &, YDMF_PHYS, YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS, YDGEOMVARS, YDCFU, YDMODEL, YL_ZFPCOR, YL_ZQV&
  &, YL_ZTENHA, YL_ZTENQVA, YL_ZTENT, YL_ZRDG_CVGQ, YL_ZRDG_CVGT, YL_ICBOT_LIG)
ELSE
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:14',0,ZHOOK_HANDLE_PARALLEL)

  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OpenMP','APL_ARPEGE_PARALLEL:14')) THEN
    
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:14:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    ICBOT_LIG => GET_HOST_DATA_RDWR (YL_ICBOT_LIG)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:14:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:14:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      ICBOT_LIG (:, JBLK)=YDCPG_OPTS%KFLEVG
    ENDDO

    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:14:COMPUTE',1,ZHOOK_HANDLE_COMPUTE)

    IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:14')) THEN
      IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:14:SYNCHOST',0,ZHOOK_HANDLE_FIELD_API)
      ICBOT_LIG => GET_HOST_DATA_RDWR (YL_ICBOT_LIG)
      IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:14:SYNCHOST',1,ZHOOK_HANDLE_FIELD_API)
      
      
    ENDIF

    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:14:NULLIFY',0,ZHOOK_HANDLE_FIELD_API)
    ICBOT_LIG => NULL ()
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:14:NULLIFY',1,ZHOOK_HANDLE_FIELD_API)
  ...
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:14',1,ZHOOK_HANDLE_PARALLEL)
ENDIF
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LECTPRODCVP) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:TURBULENCE_PART2')) THEN
    ...
    Z_YDMF_PHYS_BASE_STATE_Q_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%Q%F_P)
    Z_YDMF_PHYS_BASE_STATE_TKE_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%TKE%F_P)
    Z_YDMF_PHYS_BASE_STATE_T_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%T%F_P)
    Z_YDMF_PHYS_BASE_STATE_U_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%U%F_P)
    Z_YDMF_PHYS_BASE_STATE_V_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%V%F_P)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
    ...
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDONLY&
    & (YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
    Z_YDMF_PHYS_DIFCQ => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_DIFCQ)
    Z_YDMF_PHYS_DIFCS => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_DIFCS)
    Z_YDMF_PHYS_EDR => GET_HOST_DATA_RDWR (YDMF_PHYS%F_EDR)
    Z_YDMF_PHYS_STRCU => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_STRCU)
    Z_YDMF_PHYS_STRCV => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_STRCV)
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(:,:,&
      & JBLK), Z_YDMF_PHYS_BASE_STATE_Q_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P&
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_TKE_P(:,:, JBLK), ZPRODTH(:,:, JBLK), ZKUROV2(:,:, JBLK), Z_YDMF_PHYS_DIFCQ&
      &(:,:, JBLK), Z_YDMF_PHYS_DIFCS(:,:, JBLK), Z_YDMF_PHYS_STRCU(:,:, JBLK), Z_YDMF_PHYS_STRCV(:,:,&
      & JBLK), ZUSLE(:,:, JBLK), ZKCLS(:, JBLK), ZECTCLS(:, JBLK), ZPFL_FTKE(:,:, JBLK), ZTKE1(:,:, JBLK&
      &), Z_YDMF_PHYS_EDR(:,:, JBLK))
    ENDDO
    ...
    IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:TURBULENCE_PART2')) THEN
      ...
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR&
      & (YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
      Z_YDMF_PHYS_DIFCQ => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFCQ)
      Z_YDMF_PHYS_DIFCS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFCS)
      Z_YDMF_PHYS_EDR => GET_HOST_DATA_RDWR (YDMF_PHYS%F_EDR)
      Z_YDMF_PHYS_STRCU => GET_HOST_DATA_RDWR (YDMF_PHYS%F_STRCU)
      Z_YDMF_PHYS_STRCV => GET_HOST_DATA_RDWR (YDMF_PHYS%F_STRCV)
    ...
    ZUSLE => NULL ()
    Z_YDMF_PHYS_BASE_STATE_Q_P => NULL ()
    Z_YDMF_PHYS_BASE_STATE_TKE_P => NULL ()
    Z_YDMF_PHYS_BASE_STATE_T_P => NULL ()
    Z_YDMF_PHYS_BASE_STATE_U_P => NULL ()
    Z_YDMF_PHYS_BASE_STATE_V_P => NULL ()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL ()
    ...
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL ()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => NULL ()
    Z_YDMF_PHYS_DIFCQ => NULL ()
    Z_YDMF_PHYS_DIFCS => NULL ()
    Z_YDMF_PHYS_EDR => NULL ()
    Z_YDMF_PHYS_STRCU => NULL ()
    Z_YDMF_PHYS_STRCV => NULL ()
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMPSINGLECOLUMN&
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
        ...
        &, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF&
        &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R&
        &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(:,:, JBLK&
        &), Z_YDMF_PHYS_BASE_STATE_Q_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P&
        &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_TKE_P(:,:, JBLK), ZPRODTH(:,:, JBLK), ZKUROV2(:,:, JBLK), Z_YDMF_PHYS_DIFCQ&
        &(:,:, JBLK), Z_YDMF_PHYS_DIFCS(:,:, JBLK), Z_YDMF_PHYS_STRCU(:,:, JBLK), Z_YDMF_PHYS_STRCV(:,:, JBLK&
        &), ZUSLE(:,:, JBLK), ZKCLS(:, JBLK), ZECTCLS(:, JBLK), ZPFL_FTKE(:,:, JBLK), ZTKE1(:,:, JBLK), Z_YDMF_PHYS_EDR&
        &(:,:, JBLK),YDSTACK=YLSTACK)
      ENDDO
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN&
    ...
    Z_YDMF_PHYS_BASE_STATE_Q_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%Q%F_P)
    Z_YDMF_PHYS_BASE_STATE_TKE_P => GET_DEVICE_DATA_RDWR (YDMF_PHYS_BASE_STATE%TKE%F_P)
    Z_YDMF_PHYS_BASE_STATE_T_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%T%F_P)
    Z_YDMF_PHYS_BASE_STATE_U_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%U%F_P)
    Z_YDMF_PHYS_BASE_STATE_V_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%V%F_P)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
    ...
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_DEVICE_DATA_RDONLY&
    & (YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
    Z_YDMF_PHYS_DIFCQ => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_DIFCQ)
    Z_YDMF_PHYS_DIFCS => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_DIFCS)
    Z_YDMF_PHYS_EDR => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_EDR)
    Z_YDMF_PHYS_STRCU => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_STRCU)
    Z_YDMF_PHYS_STRCV => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_STRCV)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      ...
      !$ACC&         ZPFL_FTKE, ZPRODTH, ZTKE1, ZUSLE, Z_YDMF_PHYS_BASE_STATE_Q_P, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_TKE_P, Z_YDMF_PHYS_BASE_STATE_T_P, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_U_P, Z_YDMF_PHYS_BASE_STATE_V_P, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP, Z_YDMF_PHYS_DIFCQ, &
      !$ACC&         Z_YDMF_PHYS_DIFCS, Z_YDMF_PHYS_EDR, Z_YDMF_PHYS_STRCU, Z_YDMF_PHYS_STRCV) &
      !$ACC&PRIVATE (JBLK) &
      !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
&, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDGEOMVARS, YDMODEL, YL_ZFLU_NEIJ, YL_ZNEBS, YL_ZNEB_CVPP&
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:QNGCOR')) THEN
  ...
  & (YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
  Z_YDMF_PHYS_DIFCQ => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_DIFCQ)
  Z_YDMF_PHYS_DIFCQL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_DIFCQL)
  Z_YDMF_PHYS_DIFCQN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_DIFCQN)
  Z_YDMF_PHYS_DIFTQ => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_DIFTQ)
  Z_YDMF_PHYS_DIFTQL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFTQL)
  Z_YDMF_PHYS_DIFTQN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFTQN)
  Z_YDMF_PHYS_FCCQL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FCCQL)
  Z_YDMF_PHYS_FCCQN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FCCQN)
  Z_YDMF_PHYS_FCQNG => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FCQNG)
  Z_YDMF_PHYS_FCSQL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FCSQL)
  Z_YDMF_PHYS_FCSQN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FCSQN)
  Z_YDMF_PHYS_FPEVPCL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPEVPCL)
  Z_YDMF_PHYS_FPEVPCN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPEVPCN)
  Z_YDMF_PHYS_FPEVPSL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPEVPSL)
  Z_YDMF_PHYS_FPEVPSN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPEVPSN)
  Z_YDMF_PHYS_FPFPCL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPFPCL)
  Z_YDMF_PHYS_FPFPCN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPFPCN)
  Z_YDMF_PHYS_FPFPSL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPFPSL)
  Z_YDMF_PHYS_FPFPSN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPFPSN)
  Z_YDMF_PHYS_FPLSG => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPLSG)
  Z_YDMF_PHYS_FPLSL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPLSL)
  Z_YDMF_PHYS_FPLSN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPLSN)
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    IF (YDMODEL%YRML_PHY_MF%YRPHY%LRRGUST) THEN
      ...
      DO JLEV=YDCPG_OPTS%KTDIA-1, YDCPG_OPTS%KFLEVG
        ...
        DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
          ZPFL_FPLSH (JLON, JLEV, JBLK)=Z_YDMF_PHYS_FPLSL (JLON, JLEV, JBLK)+Z_YDMF_PHYS_FPLSN&
          & (JLON, JLEV, JBLK)+Z_YDMF_PHYS_FPLSG (JLON, JLEV, JBLK)
        ENDDO
    ...
    DO JLEV=YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG
      ...
      DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
        Z_YDMF_PHYS_DIFTQL (JLON, JLEV, JBLK)=Z_YDMF_PHYS_DIFTQL&
        & (JLON, JLEV, JBLK)+ZSEDIQL (JLON, JLEV, JBLK)
        Z_YDMF_PHYS_DIFTQN (JLON, JLEV, JBLK)=Z_YDMF_PHYS_DIFTQN&
        & (JLON, JLEV, JBLK)+ZSEDIQI (JLON, JLEV, JBLK)
        ZFPLSL (JLON, JLEV, JBLK)=Z_YDMF_PHYS_DIFTQL (JLON, JLEV, JBLK)+Z_YDMF_PHYS_FPLSL (JLON, JLEV, JBLK)
        ZFPLSN (JLON, JLEV, JBLK)=Z_YDMF_PHYS_DIFTQN (JLON, JLEV, JBLK)+Z_YDMF_PHYS_FPLSN (JLON, JLEV, JBLK)
      ENDDO
    ...
    &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:,:, JBLK), Z_YDMF_PHYS_DIFCQ(:,:, JBLK)&
    &, Z_YDMF_PHYS_DIFCQN(:,:, JBLK), Z_YDMF_PHYS_DIFCQL(:,:, JBLK), Z_YDMF_PHYS_DIFTQ(:,:, JBLK), Z_YDMF_PHYS_DIFTQN&
    &(:,:, JBLK), Z_YDMF_PHYS_DIFTQL(:,:, JBLK), Z_YDMF_PHYS_FPEVPSL(:,:, JBLK), Z_YDMF_PHYS_FPEVPSN&
    &(:,:, JBLK), Z_YDMF_PHYS_FPEVPCL(:,:, JBLK), Z_YDMF_PHYS_FPEVPCN(:,:, JBLK), Z_YDMF_PHYS_FPFPSL&
    &(:,:, JBLK), Z_YDMF_PHYS_FPFPSN(:,:, JBLK), Z_YDMF_PHYS_FPFPCL(:,:, JBLK), Z_YDMF_PHYS_FPFPCN(:&
    &,:, JBLK), Z_YDMF_PHYS_FCCQL(:,:, JBLK), Z_YDMF_PHYS_FCCQN(:,:, JBLK), Z_YDMF_PHYS_FCSQL(:,:, JBLK&
    &), Z_YDMF_PHYS_FCSQN(:,:, JBLK), Z_YDMF_PHYS_FCQNG(:,:, JBLK))
  ENDDO
  ...
  IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:QNGCOR')) THEN
    ...
    & (YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
    Z_YDMF_PHYS_DIFCQ => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFCQ)
    Z_YDMF_PHYS_DIFCQL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFCQL)
    Z_YDMF_PHYS_DIFCQN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFCQN)
    Z_YDMF_PHYS_DIFTQ => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFTQ)
    Z_YDMF_PHYS_DIFTQL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFTQL)
    Z_YDMF_PHYS_DIFTQN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_DIFTQN)
    Z_YDMF_PHYS_FCCQL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FCCQL)
    Z_YDMF_PHYS_FCCQN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FCCQN)
    Z_YDMF_PHYS_FCQNG => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FCQNG)
    Z_YDMF_PHYS_FCSQL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FCSQL)
    Z_YDMF_PHYS_FCSQN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FCSQN)
    Z_YDMF_PHYS_FPEVPCL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPEVPCL)
    Z_YDMF_PHYS_FPEVPCN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPEVPCN)
    Z_YDMF_PHYS_FPEVPSL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPEVPSL)
    Z_YDMF_PHYS_FPEVPSN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPEVPSN)
    Z_YDMF_PHYS_FPFPCL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPFPCL)
    Z_YDMF_PHYS_FPFPCN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPFPCN)
    Z_YDMF_PHYS_FPFPSL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPFPSL)
    Z_YDMF_PHYS_FPFPSN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPFPSN)
    Z_YDMF_PHYS_FPLSG => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLSG)
    Z_YDMF_PHYS_FPLSL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLSL)
    Z_YDMF_PHYS_FPLSN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLSN)
  ...
  Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => NULL ()
  Z_YDMF_PHYS_DIFCQ => NULL ()
  Z_YDMF_PHYS_DIFCQL => NULL ()
  Z_YDMF_PHYS_DIFCQN => NULL ()
  Z_YDMF_PHYS_DIFTQ => NULL ()
  Z_YDMF_PHYS_DIFTQL => NULL ()
  Z_YDMF_PHYS_DIFTQN => NULL ()
  Z_YDMF_PHYS_FCCQL => NULL ()
  Z_YDMF_PHYS_FCCQN => NULL ()
  Z_YDMF_PHYS_FCQNG => NULL ()
  Z_YDMF_PHYS_FCSQL => NULL ()
  Z_YDMF_PHYS_FCSQN => NULL ()
  Z_YDMF_PHYS_FPEVPCL => NULL ()
  Z_YDMF_PHYS_FPEVPCN => NULL ()
  Z_YDMF_PHYS_FPEVPSL => NULL ()
  Z_YDMF_PHYS_FPEVPSN => NULL ()
  Z_YDMF_PHYS_FPFPCL => NULL ()
  Z_YDMF_PHYS_FPFPCN => NULL ()
  Z_YDMF_PHYS_FPFPSL => NULL ()
  Z_YDMF_PHYS_FPFPSN => NULL ()
  Z_YDMF_PHYS_FPLSG => NULL ()
  Z_YDMF_PHYS_FPLSL => NULL ()
  Z_YDMF_PHYS_FPLSN => NULL ()
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMPSINGLECOLUMN','APL_ARPEGE_PARALLEL:QNGCOR')) THEN
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
      ...
      & YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, YDCPG_OPTS%KFLEVG, ZQV(:,:, JBLK), ZQL(:,:, JBLK&
      &), ZQI(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:,:, JBLK), Z_YDMF_PHYS_DIFCQ(:,:, JBLK&
      &), Z_YDMF_PHYS_DIFCQN(:,:, JBLK), Z_YDMF_PHYS_DIFCQL(:,:, JBLK), Z_YDMF_PHYS_DIFTQ(:,:, JBLK), Z_YDMF_PHYS_DIFTQN&
      &(:,:, JBLK), Z_YDMF_PHYS_DIFTQL(:,:, JBLK), Z_YDMF_PHYS_FPEVPSL(:,:, JBLK), Z_YDMF_PHYS_FPEVPSN(:,&
      &:, JBLK), Z_YDMF_PHYS_FPEVPCL(:,:, JBLK), Z_YDMF_PHYS_FPEVPCN(:,:, JBLK), Z_YDMF_PHYS_FPFPSL(:,:, JBLK&
      &), Z_YDMF_PHYS_FPFPSN(:,:, JBLK), Z_YDMF_PHYS_FPFPCL(:,:, JBLK), Z_YDMF_PHYS_FPFPCN(:,:, JBLK), Z_YDMF_PHYS_FCCQL&
      &(:,:, JBLK), Z_YDMF_PHYS_FCCQN(:,:, JBLK), Z_YDMF_PHYS_FCSQL(:,:, JBLK), Z_YDMF_PHYS_FCSQN(:,:, JBLK&
      &), Z_YDMF_PHYS_FCQNG(:,:, JBLK),YDSTACK=YLSTACK)
    ENDDO
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:QNGCOR')) THEN
  ...
  & (YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
  Z_YDMF_PHYS_DIFCQ => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_DIFCQ)
  Z_YDMF_PHYS_DIFCQL => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_DIFCQL)
  Z_YDMF_PHYS_DIFCQN => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_DIFCQN)
  Z_YDMF_PHYS_DIFTQ => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_DIFTQ)
  Z_YDMF_PHYS_DIFTQL => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_DIFTQL)
  Z_YDMF_PHYS_DIFTQN => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_DIFTQN)
  Z_YDMF_PHYS_FCCQL => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FCCQL)
  Z_YDMF_PHYS_FCCQN => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FCCQN)
  Z_YDMF_PHYS_FCQNG => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FCQNG)
  Z_YDMF_PHYS_FCSQL => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FCSQL)
  Z_YDMF_PHYS_FCSQN => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FCSQN)
  Z_YDMF_PHYS_FPEVPCL => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPEVPCL)
  Z_YDMF_PHYS_FPEVPCN => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPEVPCN)
  Z_YDMF_PHYS_FPEVPSL => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPEVPSL)
  Z_YDMF_PHYS_FPEVPSN => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPEVPSN)
  Z_YDMF_PHYS_FPFPCL => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPFPCL)
  Z_YDMF_PHYS_FPFPCN => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPFPCN)
  Z_YDMF_PHYS_FPFPSL => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPFPSL)
  Z_YDMF_PHYS_FPFPSN => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPFPSN)
  Z_YDMF_PHYS_FPLSG => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPLSG)
  Z_YDMF_PHYS_FPLSL => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPLSL)
  Z_YDMF_PHYS_FPLSN => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPLSN)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:QNGCOR:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:QNGCOR:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         ZSEDIQL, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP, Z_YDMF_PHYS_DIFCQ, &
    !$ACC&         Z_YDMF_PHYS_DIFCQL, Z_YDMF_PHYS_DIFCQN, Z_YDMF_PHYS_DIFTQ, &
    !$ACC&         Z_YDMF_PHYS_DIFTQL, Z_YDMF_PHYS_DIFTQN, Z_YDMF_PHYS_FCCQL, &
    !$ACC&         Z_YDMF_PHYS_FCCQN, Z_YDMF_PHYS_FCQNG, Z_YDMF_PHYS_FCSQL, &
    !$ACC&         Z_YDMF_PHYS_FCSQN, Z_YDMF_PHYS_FPEVPCL, Z_YDMF_PHYS_FPEVPCN, &
    !$ACC&         Z_YDMF_PHYS_FPEVPSL, Z_YDMF_PHYS_FPEVPSN, Z_YDMF_PHYS_FPFPCL, &
    !$ACC&         Z_YDMF_PHYS_FPFPCN, Z_YDMF_PHYS_FPFPSL, Z_YDMF_PHYS_FPFPSN, &
    !$ACC&         Z_YDMF_PHYS_FPLSG, Z_YDMF_PHYS_FPLSL, Z_YDMF_PHYS_FPLSN) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
CALL APL_ARPEGE_HYDRO_BUDGET_PARALLEL (YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDGPAR, YDMF_PHYS&
&, YDMF_PHYS_SURF, YDMODEL, YL_ZC3, YL_ZCN, YL_ZDSA_C1, YL_ZDSA_C2, YL_ZFLU_FEVI, YL_ZFLU_NEIJ,&
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:ACDRME')) THEN
  ...
  Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDONLY&
  & (YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
  Z_YDMF_PHYS_FRMH => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FRMH)
  Z_YDMF_PHYS_STRMU => GET_HOST_DATA_RDWR (YDMF_PHYS%F_STRMU)
  Z_YDMF_PHYS_STRMV => GET_HOST_DATA_RDWR (YDMF_PHYS%F_STRMV)
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(:,:, JBLK), ZQV(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P&
    &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK), Z_YDMF_PHYS_FRMH(:,:, JBLK), ZMSC_FRMQ(:,&
    &:, JBLK), Z_YDMF_PHYS_STRMU(:,:, JBLK), Z_YDMF_PHYS_STRMV(:,:, JBLK))
  ...
  Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL ()
  Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => NULL ()
  Z_YDMF_PHYS_FRMH => NULL ()
  Z_YDMF_PHYS_STRMU => NULL ()
  Z_YDMF_PHYS_STRMV => NULL ()
  Z_YDVARS_A_T1 => NULL ()
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMPSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACDRME')) THEN
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
      ...
      &%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP&
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(:,:, JBLK), ZQV(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P(:,&
      &:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK), Z_YDMF_PHYS_FRMH(:,:, JBLK), ZMSC_FRMQ(:,:, JBLK&
      &), Z_YDMF_PHYS_STRMU(:,:, JBLK), Z_YDMF_PHYS_STRMV(:,:, JBLK),YDSTACK=YLSTACK)
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACDRME')) THEN
  ...
  Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_DEVICE_DATA_RDONLY&
  & (YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
  Z_YDMF_PHYS_FRMH => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_FRMH)
  Z_YDMF_PHYS_STRMU => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_STRMU)
  Z_YDMF_PHYS_STRMV => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_STRMV)
  ...
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACDRME:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_T_P, Z_YDMF_PHYS_BASE_STATE_U_P, &
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_V_P, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP, &
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP, Z_YDMF_PHYS_FRMH, &
    !$ACC&         Z_YDMF_PHYS_STRMU, Z_YDMF_PHYS_STRMV, Z_YDVARS_A_T1, Z_YDVARS_IRAD_T1, &
    !$ACC&         Z_YDVARS_LRAD_T1) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
IF (YDMODEL%YRML_PHY_MF%YRPHY2%LRAFTKE) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:ACEVADCAPE')) THEN
    ...
    ICBOT_LIG => GET_HOST_DATA_RDONLY (YL_ICBOT_LIG)
    IMOC_CLPH => GET_HOST_DATA_RDWR (YL_IMOC_CLPH)
    ZBLH => GET_HOST_DATA_RDWR (YL_ZBLH)
    ZDCAPE => GET_HOST_DATA_RDWR (YL_ZDCAPE)
    ZTKE1 => GET_HOST_DATA_RDONLY (YL_ZTKE1)
    Z_YDGEOMVARS_OROG => GET_HOST_DATA_RDONLY (YDGEOMVARS%F_OROG)
    Z_YDMF_PHYS_BASE_STATE_T_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%T%F_P)
    Z_YDMF_PHYS_BASE_STATE_U_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%U%F_P)
    Z_YDMF_PHYS_BASE_STATE_V_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%V%F_P)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
    ...
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDONLY&
    & (YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDMF_PHYS_CAPE => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_CAPE)
    Z_YDMF_PHYS_CLPH => GET_HOST_DATA_RDWR (YDMF_PHYS%F_CLPH)
    Z_YDMF_PHYS_FPLCL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPLCL)
    Z_YDMF_PHYS_FPLCN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPLCN)
    Z_YDMF_PHYS_FPLSL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPLSL)
    Z_YDMF_PHYS_FPLSN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPLSN)
    Z_YDMF_PHYS_UCLS => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_UCLS)
    Z_YDMF_PHYS_UGST => GET_HOST_DATA_RDWR (YDMF_PHYS%F_UGST)
    Z_YDMF_PHYS_VCLS => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_VCLS)
    Z_YDMF_PHYS_VGST => GET_HOST_DATA_RDWR (YDMF_PHYS%F_VGST)
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      CALL ACEVADCAPE (YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRCST, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,&
      & YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_FPLSL(:,:, JBLK), Z_YDMF_PHYS_FPLSN(:,:, JBLK&
      &), Z_YDMF_PHYS_FPLCL(:,:, JBLK), Z_YDMF_PHYS_FPLCN(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD&
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(:,&
      &:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF&
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:,:, JBLK), ZDCAPE(:, JBLK))
      CALL ACCLDIA (YDMODEL%YRCST, YDXFU%LXCLP, YDXFU%LXTGST, YDXFU%LXXGST, YDMODEL%YRML_PHY_MF&
      &%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRML_PHY_MF%YRTOPH, YLCPG_BNDS%KIDIA, YLCPG_BNDS&
      &%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_UCLS(:, JBLK), Z_YDMF_PHYS_VCLS&
      &(:, JBLK), Z_YDMF_PHYS_BASE_STATE_U_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK)&
      &, Z_YDMF_PHYS_CAPE(:, JBLK), ICBOT_LIG(:, JBLK), ZDCAPE(:, JBLK), ZTKE1(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF&
      &(:,:, JBLK), Z_YDGEOMVARS_OROG(:, JBLK), Z_YDMF_PHYS_UGST(:, JBLK), Z_YDMF_PHYS_VGST(:,&
      & JBLK), ZBLH(:, JBLK), IMOC_CLPH(:, JBLK))

      DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
        Z_YDMF_PHYS_CLPH (JLON, JBLK)=MIN (YDMODEL%YRML_PHY_MF%YRPHY0%XMAXLM&
    ...
    IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:ACEVADCAPE')) THEN
      ...
      ZDCAPE => GET_HOST_DATA_RDWR (YL_ZDCAPE)
      ZTKE1 => GET_HOST_DATA_RDWR (YL_ZTKE1)
      Z_YDGEOMVARS_OROG => GET_HOST_DATA_RDWR (YDGEOMVARS%F_OROG)
      Z_YDMF_PHYS_BASE_STATE_T_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%T%F_P)
      Z_YDMF_PHYS_BASE_STATE_U_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%U%F_P)
      Z_YDMF_PHYS_BASE_STATE_V_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%V%F_P)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      ...
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR&
      & (YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_CAPE => GET_HOST_DATA_RDWR (YDMF_PHYS%F_CAPE)
      Z_YDMF_PHYS_CLPH => GET_HOST_DATA_RDWR (YDMF_PHYS%F_CLPH)
      Z_YDMF_PHYS_FPLCL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLCL)
      Z_YDMF_PHYS_FPLCN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLCN)
      Z_YDMF_PHYS_FPLSL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLSL)
      Z_YDMF_PHYS_FPLSN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLSN)
      Z_YDMF_PHYS_UCLS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_UCLS)
      Z_YDMF_PHYS_UGST => GET_HOST_DATA_RDWR (YDMF_PHYS%F_UGST)
    ...
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL ()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL ()
    Z_YDMF_PHYS_CAPE => NULL ()
    Z_YDMF_PHYS_CLPH => NULL ()
    Z_YDMF_PHYS_FPLCL => NULL ()
    Z_YDMF_PHYS_FPLCN => NULL ()
    Z_YDMF_PHYS_FPLSL => NULL ()
    Z_YDMF_PHYS_FPLSN => NULL ()
    Z_YDMF_PHYS_UCLS => NULL ()
    Z_YDMF_PHYS_UGST => NULL ()
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMPSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACEVADCAPE')) THEN
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
        ...
        &, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_FPLSL(:,:, JBLK), Z_YDMF_PHYS_FPLSN(:,:, JBLK), Z_YDMF_PHYS_FPLCL&
        &(:,:, JBLK), Z_YDMF_PHYS_FPLCN(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF&
        &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:,:, JBLK&
        &), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:,:, JBLK)&
        &, ZDCAPE(:, JBLK),YDSTACK=YLSTACK)
        CALL ACCLDIA_OPENACC (YDMODEL%YRCST, YDXFU%LXCLP, YDXFU%LXTGST, YDXFU%LXXGST, YDMODEL%YRML_PHY_MF&
        &%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRML_PHY_MF%YRTOPH, YLCPG_BNDS%KIDIA, YLCPG_BNDS&
        &%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_UCLS(:, JBLK), Z_YDMF_PHYS_VCLS(:,&
        & JBLK), Z_YDMF_PHYS_BASE_STATE_U_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_V_P(:,:, JBLK), Z_YDMF_PHYS_CAPE&
        &(:, JBLK), ICBOT_LIG(:, JBLK), ZDCAPE(:, JBLK), ZTKE1(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF&
        &(:,:, JBLK), Z_YDGEOMVARS_OROG(:, JBLK), Z_YDMF_PHYS_UGST(:, JBLK), Z_YDMF_PHYS_VGST(:, JBLK&
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACEVADCAPE')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACEVADCAPE:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    ICBOT_LIG => GET_DEVICE_DATA_RDONLY (YL_ICBOT_LIG)
    IMOC_CLPH => GET_DEVICE_DATA_RDWR (YL_IMOC_CLPH)
    ZBLH => GET_DEVICE_DATA_RDWR (YL_ZBLH)
    ZDCAPE => GET_DEVICE_DATA_RDWR (YL_ZDCAPE)
    ZTKE1 => GET_DEVICE_DATA_RDONLY (YL_ZTKE1)
    Z_YDGEOMVARS_OROG => GET_DEVICE_DATA_RDONLY (YDGEOMVARS%F_OROG)
    Z_YDMF_PHYS_BASE_STATE_T_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%T%F_P)
    Z_YDMF_PHYS_BASE_STATE_U_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%U%F_P)
    Z_YDMF_PHYS_BASE_STATE_V_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%V%F_P)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
    ...
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_DEVICE_DATA_RDONLY&
    & (YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDMF_PHYS_CAPE => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_CAPE)
    Z_YDMF_PHYS_CLPH => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_CLPH)
    Z_YDMF_PHYS_FPLCL => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPLCL)
    Z_YDMF_PHYS_FPLCN => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPLCN)
    Z_YDMF_PHYS_FPLSL => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPLSL)
    Z_YDMF_PHYS_FPLSN => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_FPLSN)
    Z_YDMF_PHYS_UCLS => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_UCLS)
    Z_YDMF_PHYS_UGST => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_UGST)
    Z_YDMF_PHYS_VCLS => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_VCLS)
    Z_YDMF_PHYS_VGST => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_VGST)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACEVADCAPE:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACEVADCAPE:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      ...
      !$ACC PARALLEL LOOP GANG &
      !$ACC&PRESENT (ICBOT_LIG, IMOC_CLPH, YDCPG_OPTS, YDMODEL, YDXFU, YFXTRAN_ACDC_STACK, &
      !$ACC&         ZBLH, ZDCAPE, ZTKE1, Z_YDGEOMVARS_OROG, Z_YDMF_PHYS_BASE_STATE_T_P, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_U_P, Z_YDMF_PHYS_BASE_STATE_V_P, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD, &
      !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDMF_PHYS_CAPE, &
      !$ACC&         Z_YDMF_PHYS_CLPH, Z_YDMF_PHYS_FPLCL, Z_YDMF_PHYS_FPLCN, Z_YDMF_PHYS_FPLSL, &
      !$ACC&         Z_YDMF_PHYS_FPLSN, Z_YDMF_PHYS_UCLS, Z_YDMF_PHYS_UGST, Z_YDMF_PHYS_VCLS, &
      !$ACC&         Z_YDMF_PHYS_VGST) &
      !$ACC&PRIVATE (JBLK) &
      !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:ACVISIH')) THEN
  ...
  Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDONLY&
  & (YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
  Z_YDMF_PHYS_MXCLWC => GET_HOST_DATA_RDWR (YDMF_PHYS%F_MXCLWC)
  Z_YDMF_PHYS_VISICLD => GET_HOST_DATA_RDWR (YDMF_PHYS%F_VISICLD)
  Z_YDMF_PHYS_VISIHYD => GET_HOST_DATA_RDWR (YDMF_PHYS%F_VISIHYD)
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(:,:, JBLK&
    &), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:,:, JBLK), Z_YDCPG_MISC_QLI(:,:, JBLK), Z_YDCPG_MISC_QICE&
    &(:,:, JBLK), ZQR(:,:, JBLK), ZQS(:,:, JBLK), ZQGM(:,:, JBLK), Z_YDMF_PHYS_VISICLD(:, JBLK), Z_YDMF_PHYS_VISIHYD&
    &(:, JBLK), Z_YDMF_PHYS_MXCLWC(:, JBLK))
  ENDDO
  ...
  Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL ()
  Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL ()
  Z_YDMF_PHYS_MXCLWC => NULL ()
  Z_YDMF_PHYS_VISICLD => NULL ()
  Z_YDMF_PHYS_VISIHYD => NULL ()
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMPSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACVISIH')) THEN
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
      ...
      CALL ACVISIH_OPENACC (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI, YLCPG_BNDS%KIDIA,&
      & YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI&
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF&
      &(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_T_P(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R&
      &(:,:, JBLK), Z_YDCPG_MISC_QLI(:,:, JBLK), Z_YDCPG_MISC_QICE(:,:, JBLK), ZQR(:,:, JBLK), ZQS&
      &(:,:, JBLK), ZQGM(:,:, JBLK), Z_YDMF_PHYS_VISICLD(:, JBLK), Z_YDMF_PHYS_VISIHYD(:, JBLK)&
      &, Z_YDMF_PHYS_MXCLWC(:, JBLK),YDSTACK=YLSTACK)
    ENDDO
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:ACVISIH')) THEN
  ...
  Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_DEVICE_DATA_RDONLY&
  & (YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
  Z_YDMF_PHYS_MXCLWC => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_MXCLWC)
  Z_YDMF_PHYS_VISICLD => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_VISICLD)
  Z_YDMF_PHYS_VISIHYD => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_VISIHYD)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACVISIH:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ACVISIH:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         Z_YDCPG_MISC_QLI, Z_YDMF_PHYS_BASE_STATE_T_P, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, &
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R, &
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDMF_PHYS_MXCLWC, &
    !$ACC&         Z_YDMF_PHYS_VISICLD, Z_YDMF_PHYS_VISIHYD) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:ZBAY_QRCONV')) THEN
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JLEV=1, YDCPG_OPTS%KFLEVG
      ...
      DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
        ZBAY_QRCONV (JLON, JLEV, JBLK)=0.001_JPRB*((ABS (Z_YDMF_PHYS_FPLCL (JLON, JLEV, JBLK))*YDMODEL&
        &%YRML_PHY_MF%YRPHY0%RCOEFRAIN (1))**YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFRAIN (2))
        ZBAY_QSCONV (JLON, JLEV, JBLK)=0.001_JPRB*((ABS (Z_YDMF_PHYS_FPLCN (JLON, JLEV, JBLK))*YDMODEL&
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:ZBAY_QRCONV')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         Z_YDMF_PHYS_FPLCL, Z_YDMF_PHYS_FPLCN) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:ZDE2MR')) THEN
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JLEV=1, YDCPG_OPTS%KFLEVG
      ...
      DO JLON=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
        ZDE2MR (JLON, JLEV, JBLK)=YDMODEL%YRCST%RD*Z_YDMF_PHYS_BASE_STATE_T_P(JLON&
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:ZDE2MR')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:ZDE2MR:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC PARALLEL LOOP GANG &
    !$ACC&PRESENT (YDCPG_OPTS, YDMODEL, YFXTRAN_ACDC_STACK, ZBAY_QRCONV, ZBAY_QSCONV, &
    !$ACC&         ZDE2MR, Z_YDMF_PHYS_BASE_STATE_T_P, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:PPWETPOINT')) THEN
  ...
  Z_YDMF_PHYS_BASE_STATE_I_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%I%F_P)
  Z_YDMF_PHYS_BASE_STATE_L_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%L%F_P)
  Z_YDMF_PHYS_BASE_STATE_Q_P => GET_HOST_DATA_RDONLY (YDMF_PHYS_BASE_STATE%Q%F_P)
  Z_YDMF_PHYS_TCLS => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_TCLS)
  Z_YDMF_PHYS_TPWCLS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_TPWCLS)
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    &, YDCPG_OPTS%KLON, 1, ZPCLS(:, JBLK), Z_YDMF_PHYS_TCLS(:, JBLK), Z_YDMF_PHYS_BASE_STATE_Q_P&
    & (:, YDCPG_OPTS%KFLEVG, JBLK), Z_YDMF_PHYS_BASE_STATE_L_P (:, YDCPG_OPTS%KFLEVG, JBLK), Z_YDMF_PHYS_BASE_STATE_I_P&
    & (:, YDCPG_OPTS%KFLEVG, JBLK), Z_YDMF_PHYS_TPWCLS(:, JBLK))
  ENDDO
  ...
  IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:PPWETPOINT')) THEN
    ...
    ZPCLS => GET_HOST_DATA_RDWR (YL_ZPCLS)
    Z_YDMF_PHYS_BASE_STATE_I_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%I%F_P)
    Z_YDMF_PHYS_BASE_STATE_L_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%L%F_P)
    Z_YDMF_PHYS_BASE_STATE_Q_P => GET_HOST_DATA_RDWR (YDMF_PHYS_BASE_STATE%Q%F_P)
    Z_YDMF_PHYS_TCLS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_TCLS)
    Z_YDMF_PHYS_TPWCLS => GET_HOST_DATA_RDWR (YDMF_PHYS%F_TPWCLS)
  ...
  ZPCLS => NULL ()
  Z_YDMF_PHYS_BASE_STATE_I_P => NULL ()
  Z_YDMF_PHYS_BASE_STATE_L_P => NULL ()
  Z_YDMF_PHYS_BASE_STATE_Q_P => NULL ()
  Z_YDMF_PHYS_TCLS => NULL ()
  Z_YDMF_PHYS_TPWCLS => NULL ()
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMPSINGLECOLUMN','APL_ARPEGE_PARALLEL:PPWETPOINT')) THEN
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
      ...
      &, YDCPG_OPTS%KLON, 1, ZPCLS(:, JBLK), Z_YDMF_PHYS_TCLS(:, JBLK), Z_YDMF_PHYS_BASE_STATE_Q_P (:, YDCPG_OPTS&
      &%KFLEVG, JBLK), Z_YDMF_PHYS_BASE_STATE_L_P (:, YDCPG_OPTS%KFLEVG, JBLK), Z_YDMF_PHYS_BASE_STATE_I_P&
      & (:, YDCPG_OPTS%KFLEVG, JBLK), Z_YDMF_PHYS_TPWCLS(:, JBLK),YDSTACK=YLSTACK)
    ENDDO
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:PPWETPOINT')) THEN
  ...
  Z_YDMF_PHYS_BASE_STATE_I_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%I%F_P)
  Z_YDMF_PHYS_BASE_STATE_L_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%L%F_P)
  Z_YDMF_PHYS_BASE_STATE_Q_P => GET_DEVICE_DATA_RDONLY (YDMF_PHYS_BASE_STATE%Q%F_P)
  Z_YDMF_PHYS_TCLS => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_TCLS)
  Z_YDMF_PHYS_TPWCLS => GET_DEVICE_DATA_RDWR (YDMF_PHYS%F_TPWCLS)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:PPWETPOINT:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:PPWETPOINT:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
  
  
    !$ACC PARALLEL LOOP GANG &
    !$ACC&PRESENT (YDCPG_OPTS, YDMODEL, YFXTRAN_ACDC_STACK, ZPCLS, Z_YDMF_PHYS_BASE_STATE_I_P, &
    !$ACC&         Z_YDMF_PHYS_BASE_STATE_L_P, Z_YDMF_PHYS_BASE_STATE_Q_P, &
    !$ACC&         Z_YDMF_PHYS_TCLS, Z_YDMF_PHYS_TPWCLS) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
& YDMF_PHYS, YDVARS, YDGEOMVARS, YDMODEL, YL_ZPRC_DPRECIPS, YL_ZPRC_DPRECIPS2)
...
IF (YDCPG_OPTS%LCORWAT) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OpenMP','APL_ARPEGE_PARALLEL:23')) THEN
    ...
    Z_YDMF_PHYS_FEVL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FEVL)
    Z_YDMF_PHYS_FEVN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FEVN)
    Z_YDMF_PHYS_FPLCL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPLCL)
    Z_YDMF_PHYS_FPLCN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPLCN)
    Z_YDMF_PHYS_FPLSL => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPLSL)
    Z_YDMF_PHYS_FPLSN => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_FPLSN)
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      CALL MF_PHYS_CORWAT (YLCPG_BNDS, YDCPG_OPTS, Z_YDMF_PHYS_FEVL(:,:, JBLK), Z_YDMF_PHYS_FEVN&
      &(:,:, JBLK), Z_YDMF_PHYS_FPLCL(:,:, JBLK), Z_YDMF_PHYS_FPLCN(:,:, JBLK), Z_YDMF_PHYS_FPLSL&
      &(:,:, JBLK), Z_YDMF_PHYS_FPLSN(:,:, JBLK), Z_YDMF_PHYS_SURF_GSD_VH_PEVA(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VH_PPCL&
      &(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VH_PPCN(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VH_PPSL(:, JBLK&
      &), Z_YDMF_PHYS_SURF_GSD_VH_PPSN(:, JBLK))
    ENDDO
    ...
    IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:23')) THEN
      ...
      Z_YDMF_PHYS_FEVL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FEVL)
      Z_YDMF_PHYS_FEVN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FEVN)
      Z_YDMF_PHYS_FPLCL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLCL)
      Z_YDMF_PHYS_FPLCN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLCN)
      Z_YDMF_PHYS_FPLSL => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLSL)
      Z_YDMF_PHYS_FPLSN => GET_HOST_DATA_RDWR (YDMF_PHYS%F_FPLSN)
    ...
    Z_YDMF_PHYS_FEVL => NULL ()
    Z_YDMF_PHYS_FEVN => NULL ()
    Z_YDMF_PHYS_FPLCL => NULL ()
    Z_YDMF_PHYS_FPLCN => NULL ()
    Z_YDMF_PHYS_FPLSL => NULL ()
    Z_YDMF_PHYS_FPLSN => NULL ()
...
&, YDVARS, YDGEOMVARS, YDMODEL, YDDDH, YL_ZDIFEXT, YL_ZDSA_CPS, YL_ZMSC_FRMQ, YL_ZPFL_FEFB1&
&, YL_ZPFL_FEFB2, YL_ZPFL_FEFB3, YL_ZPFL_FTKE, YL_ZPFL_FTKEI, YL_ZFCHQ)
...
CALL APL_ARPEGE_SURFACE_UPDATE_PARALLEL (YDCPG_BNDS, YDCPG_OPTS, YDGPAR, YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL&
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVPGY) THEN
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:25',0,ZHOOK_HANDLE_PARALLEL)

  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OpenMP','APL_ARPEGE_PARALLEL:25')) THEN
    
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:25:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    Z_YDVARS_CVV_T0 => GET_HOST_DATA_RDONLY (YDVARS%CVV%FT0)
    Z_YDVARS_CVV_T1 => GET_HOST_DATA_RDWR (YDVARS%CVV%FT1)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:25:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:25:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:25:COMPUTE',1,ZHOOK_HANDLE_COMPUTE)

    IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_PARALLEL:25')) THEN
      IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:25:SYNCHOST',0,ZHOOK_HANDLE_FIELD_API)
      Z_YDVARS_CVV_T0 => GET_HOST_DATA_RDWR (YDVARS%CVV%FT0)
      Z_YDVARS_CVV_T1 => GET_HOST_DATA_RDWR (YDVARS%CVV%FT1)
      IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:25:SYNCHOST',1,ZHOOK_HANDLE_FIELD_API)
      
      
    ENDIF

    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:25:NULLIFY',0,ZHOOK_HANDLE_FIELD_API)
    Z_YDVARS_CVV_T0 => NULL ()
    Z_YDVARS_CVV_T1 => NULL ()
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:25:NULLIFY',1,ZHOOK_HANDLE_FIELD_API)
  ...
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:25',1,ZHOOK_HANDLE_PARALLEL)
ENDIF
...
IF (YDCPG_OPTS%LCONFX) THEN
  ...
  &%GSD_VV%F_HV, YDMF_PHYS_SURF%GSD_VV%F_Z0H, YDVARS%DAL%FT0, YDVARS%DOM%FT0, YDVARS%UAL%FT0, YDVARS%UEN&
  &%FT0, YDVARS%UNEBH%FT0, YDVARS%UOM%FT0, YDVARS%MO%FT0, YDMODEL)
  CALL TRANSFER_CLOUD_FIELDS_PARALLEL (YDMODEL, YDVARS, YDCPG_OPTS, YDCPG_BNDS)
ENDIF
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_PARALLEL:EDR')) THEN
  ...
  Z_YDMF_PHYS_EDR => GET_HOST_DATA_RDONLY (YDMF_PHYS%F_EDR)
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    IF (YDMODEL%YRML_PHY_MF%YRPHY%LEDR) THEN
      ...
      DO JLEV=1, YDCPG_OPTS%YRSURF_DIMS%YSD_DID%NLEVS
        Z_YDMF_PHYS_SURF_GSD_DI_PXEDR (:, JLEV, JBLK)=Z_YDMF_PHYS_EDR (:, JLEV, JBLK)
      ENDDO
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
      ...
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LEDR) THEN
        ...
        DO JLEV=1, YDCPG_OPTS%YRSURF_DIMS%YSD_DID%NLEVS
          Z_YDMF_PHYS_SURF_GSD_DI_PXEDR (JLON, JLEV, JBLK)=Z_YDMF_PHYS_EDR (JLON, JLEV, JBLK)
        ENDDO
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_PARALLEL:EDR')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:EDR:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
  Z_YDMF_PHYS_EDR => GET_DEVICE_DATA_RDONLY (YDMF_PHYS%F_EDR)
  ...
  IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PARALLEL:EDR:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC PARALLEL LOOP GANG &
    !$ACC&PRESENT (YDCPG_OPTS, YDMODEL, YFXTRAN_ACDC_STACK, Z_YDMF_PHYS_EDR, Z_YDMF_PHYS_SURF_GSD_DI_PXEDR) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
CALL FXTRAN_ACDC_GPUMEMSTAT ("apl_arpege_parallel.F90", 999, "END")
...
IF (ASSOCIATED (YL_ZSAV_MO)) CALL FIELD_DELETE (YL_ZSAV_MO)
