...
type spp_model
  ...
  character(len=10)  :: summary='ifs'    ! which settings to show in the summary
end type spp_model
...
subroutine implement_spp_pertn( sm, cdlabel, pxmag, pxmin, pxmax, xclipmin, xclipmax, ic, &
...
subroutine write_spp_pn_infoline( p, summary, ku)
  ...
  ! Labels
  character(len=10), parameter :: cdistr(0:2) = (/'normal    ','log-normal','uniform   '/)
  real(kind=jphook)  :: zhook_handle
  ...
  select case(TRIM(summary))
   case('lam')
    write( iunit, '(A16,X,F10.5,X,L3,2(X,I3),X,A10,X,F10.5,X,I11,2(X,e11.4))' ) &
         & p%label, p%xmag(1), p%ln1, &
         & p%mp, p%nrf, cdistr(p%idistr), p%xuniform_offset, p%nseed_off(1), &
         & p%xclipmin,p%xclipmax
   case default
...
subroutine write_spp_model_table( sm, ku, ldefined )
  ...
  do jjpert=1, inperts
    if (lldef) then
      call write_spp_pn_infoline( sm%pndef(jjpert), sm%summary, ku=iunit)
    else
      call write_spp_pn_infoline( sm%pn(jjpert), sm%summary, ku=iunit)
    endif
  enddo
  write( iunit, '("----------------------------------------------------------------------------------------------------------&
       &--------------------------------------------------------------------------------------------------------------")' )
