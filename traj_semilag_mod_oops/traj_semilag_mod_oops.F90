...
SUBROUTINE ALLOCATE_TRAJ_SLAG_OOPS(SELF,NPROMA,NFLEVG,NGPBLKS,YGFL,YDDYN)
USE YOMDYN   , ONLY : TDYN
USE YOM_YGFL , ONLY : TYPE_GFLD
IMPLICIT NONE
INTEGER(KIND=JPIM)  , INTENT(IN)    :: NPROMA, NFLEVG,NGPBLKS
...
DO JBLK=1,NGPBLKS
  ...
  IF (LR2D) THEN ! Clive's code for shallow water model
    ...
    IF (NTRSLTYPE == 1) THEN
      ALLOCATE(SELF(JBLK)%PUT95(NPROMA)) 
      ALLOCATE(SELF(JBLK)%PVT95(NPROMA)) 
      ALLOCATE(SELF(JBLK)%PSPNLT95(NPROMA)) 
    ELSEIF(NTRSLTYPE == 2) THEN
      ALLOCATE(SELF(JBLK)%PUL95(NPROMA,1)) 
      ALLOCATE(SELF(JBLK)%PVL95(NPROMA,1)) 
      ALLOCATE(SELF(JBLK)%PUL05(NPROMA,1)) 
      ALLOCATE(SELF(JBLK)%PVL05(NPROMA,1)) 
      ALLOCATE(SELF(JBLK)%PURL05(NPROMA,1)) 
      ALLOCATE(SELF(JBLK)%PVRL05(NPROMA,1)) 
      ALLOCATE(SELF(JBLK)%PURL5(NPROMA,1)) 
      ALLOCATE(SELF(JBLK)%PVRL5(NPROMA,1)) 
      ALLOCATE(SELF(JBLK)%PUSI5(NPROMA)) 
      ALLOCATE(SELF(JBLK)%PVSI5(NPROMA)) 
      ALLOCATE(SELF(JBLK)%PUT15(NPROMA,1)) 
      ALLOCATE(SELF(JBLK)%PVT15(NPROMA,1)) 
      ALLOCATE(SELF(JBLK)%PSPL95(NPROMA)) 
      ALLOCATE(SELF(JBLK)%PSPL05(NPROMA)) 
    ENDIF
  
  ELSE  ! full 3D model
  
    ALLOCATE(SELF(JBLK)%PUL95(NPROMA,NFLEVG)) 
    SELF(JBLK)%PUL95 = 0.
    ALLOCATE(SELF(JBLK)%PVL95(NPROMA,NFLEVG)) 
    SELF(JBLK)%PVL95 = 0.
    ALLOCATE(SELF(JBLK)%PTL95(NPROMA,NFLEVG)) 
    SELF(JBLK)%PTL95 = 0.
    ALLOCATE(SELF(JBLK)%PCL95(NPROMA,NFLEVG)) 
    SELF(JBLK)%PCL95 = 0.
    ALLOCATE(SELF(JBLK)%PUL05(NPROMA,NFLEVG)) 
    SELF(JBLK)%PUL05 = 0.
    ALLOCATE(SELF(JBLK)%PVL05(NPROMA,NFLEVG)) 
    SELF(JBLK)%PVL05 = 0.
    ALLOCATE(SELF(JBLK)%PTL05(NPROMA,NFLEVG)) 
    SELF(JBLK)%PTL05 = 0.
    ALLOCATE(SELF(JBLK)%PURL05(NPROMA,NFLEVG)) 
    SELF(JBLK)%PURL05 = 0.
    ALLOCATE(SELF(JBLK)%PVRL05(NPROMA,NFLEVG)) 
    SELF(JBLK)%PVRL05 = 0.
    ALLOCATE(SELF(JBLK)%PWRL05(NPROMA,NFLEVG)) 
    SELF(JBLK)%PWRL05 = 0.
    ALLOCATE(SELF(JBLK)%PURL5(NPROMA,NFLEVG)) 
    SELF(JBLK)%PURL5 = 0.
    ALLOCATE(SELF(JBLK)%PVRL5(NPROMA,NFLEVG)) 
    SELF(JBLK)%PVRL5 = 0.
    ALLOCATE(SELF(JBLK)%PWRL5(NPROMA,NFLEVG)) 
    SELF(JBLK)%PWRL5 = 0.
    IF (YDDYN%LADVF.AND.YDDYN%L2TLFF) THEN
      ALLOCATE(SELF(JBLK)%PUT15(NPROMA,NFLEVG)) 
      SELF(JBLK)%PUT15 = 0.
      ALLOCATE(SELF(JBLK)%PVT15(NPROMA,NFLEVG)) 
      SELF(JBLK)%PVT15 = 0.
    ENDIF
    IF (LSPRT) THEN
      ALLOCATE(SELF(JBLK)%PTT15(NPROMA,NFLEVG)) 
      SELF(JBLK)%PTT15 = 0.
      ALLOCATE(SELF(JBLK)%PQT15(NPROMA,NFLEVG))
      SELF(JBLK)%PQT15 = 0.
    ENDIF
    IF (LSLHD.AND.(.NOT.LSLHD_STATIC)) THEN
      ALLOCATE(SELF(JBLK)%PKAPPA5(NPROMA,NFLEVG)) 
      SELF(JBLK)%PKAPPA5 = 0.
      IF (YDDYN%LSLHDHEAT) THEN
         ALLOCATE(SELF(JBLK)%PKAPPAT5(NPROMA,NFLEVG))
         SELF(JBLK)%PKAPPAT5 = 0.
      ENDIF
    ENDIF
    IF (YDDYN%LSETTLSVF) THEN
      ALLOCATE(SELF(JBLK)%PWRL95(NPROMA,NFLEVG)) 
      SELF(JBLK)%PWRL95 = 0.
    ENDIF
    ALLOCATE(SELF(JBLK)%PX95(NPROMA))
    SELF(JBLK)%PX95 = 0.

    ALLOCATE(SELF(JBLK)%PGFLL95(NPROMA,NFLEVG,IGFLADV))
    SELF(JBLK)%PGFLL95 = 0.
...
IF (ANY(ISNAN(SELF%PUL95   ))) WRITE(0,*) 'semilag traj NaN in PUL95    in blk ',KBLKNUM 
IF (ANY(ISNAN(SELF%PVL95   ))) WRITE(0,*) 'semilag traj NaN in PVL95    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PTL95   ))) WRITE(0,*) 'semilag traj NaN in PTL95    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PCL95   ))) WRITE(0,*) 'semilag traj NaN in PCL95    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PUL05   ))) WRITE(0,*) 'semilag traj NaN in PUL05    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PVL05   ))) WRITE(0,*) 'semilag traj NaN in PVL05    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PTL05   ))) WRITE(0,*) 'semilag traj NaN in PTL05    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PUT15   ))) WRITE(0,*) 'semilag traj NaN in PUT15    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PVT15   ))) WRITE(0,*) 'semilag traj NaN in PVT15    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PURL05  ))) WRITE(0,*) 'semilag traj NaN in PURL05   in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PVRL05  ))) WRITE(0,*) 'semilag traj NaN in PVRL05   in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PWRL05  ))) WRITE(0,*) 'semilag traj NaN in PWRL05   in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PURL5   ))) WRITE(0,*) 'semilag traj NaN in PURL5    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PVRL5   ))) WRITE(0,*) 'semilag traj NaN in PVRL5    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PWRL5   ))) WRITE(0,*) 'semilag traj NaN in PWRL5    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PKAPPA5 ))) WRITE(0,*) 'semilag traj NaN in PKAPPA5) in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PKAPPAT5))) WRITE(0,*) 'semilag traj NaN in PKAPPAT5 in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PTT15   ))) WRITE(0,*) 'semilag traj NaN in PTT15    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PQT15   ))) WRITE(0,*) 'semilag traj NaN in PQT15    in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PWRL95  ))) WRITE(0,*) 'semilag traj NaN in PWRL95   in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PGFLL95 ))) WRITE(0,*) 'semilag traj NaN in PGFLL95  in blk ',KBLKNUM
IF (ANY(ISNAN(SELF%PX95    ))) WRITE(0,*) 'semilag traj NaN in PX95     in blk ',KBLKNUM
